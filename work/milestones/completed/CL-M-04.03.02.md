...
Each phase follows FlowTime guardrails (TDD order, tracker updates, full `dotnet build` + `dotnet test` before hand-off). Adoption guidance and analyzer output must be recorded in `work/milestones/tracking/CL-M-04.03.02-tracking.md` when the milestone kicks off.
