...
2. **ID stability.** ✅ **Settled — best-effort per-epic preprocessing + chronological renumber for D/G.**
   ...
   **Milestone ids.** Use `id: auto` in the manifest; aiwf allocates `M-001..M-NNN` in deterministic manifest order (epic ascending, then milestone ascending within each epic). Old `m-E18-13-session-evaluator` → new `M-053` (or whatever falls out). Mapping captured in `id-map.csv`. Body cross-references rewritten via the map.
   ...
   **Gap ids.** 157 entries in `work/gaps.md`, only 8 with explicit dates (and those are *resolution* dates). Use **`git blame`** on each H2 line to recover the entry's creation date (the commit that introduced it). Sort by (blame-date, blame-commit-time) → assign `G-001..G-157`. Verified by sampling: blame produces clean per-entry dates ranging 2026-03-24..2026-04-28; doc order is unreliable (recent entries inserted at top).
