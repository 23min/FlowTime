...
**Consequences:**
- Rust engine core gaps (per-class decomposition, edge series, `outputs:` filtering) are evaluation-layer work and must land before E-0017/E-0018.
...
- For E-0018 pipeline mode: `compile(model) → Plan; for params in sweep: eval(plan, params) → EvalResult; sink.write(...); yield to consumer`. The sink writes each result as it arrives — crash at iteration N means N-1 results are persisted. The sink and consumer run in parallel on the same EvalResult.
