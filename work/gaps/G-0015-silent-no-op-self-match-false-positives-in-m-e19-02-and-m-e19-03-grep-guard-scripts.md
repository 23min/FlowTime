...
Neither layer represents a real regression on the `main` tree: every underlying M-0025 and M-0026 cleanup invariant (stored drafts gone, ZIP archive gone, `POST /v1/runs` bundle-import gone, catalogs gone, `binMinutes` authoring shape gone, schema-migration examples archived, `docs/ui/template-integration-spec.md` archived, catalog-stale phrasing cleaned) is in fact intact on current `main`, verified by hand.
...
1. **Fail-fast `command -v rg` check** backported from `scripts/m-E19-04-grep-guards.sh` to the M-0025 and M-0026 scripts. Silent no-ops on machines without ripgrep are now loud failures with an install hint.
...
- **When writing future grep-guard scripts, use the M-0027 template** (fail-fast `command -v rg` + `--glob` exclusions that keep the script body out of its own search path) as the reference.
- **Global grep patterns cannot enforce handler-scoped invariants.** If an AC's real invariant is "no X in handler Y", a global grep across the containing file will false-positive on any other handler that legitimately uses X. Either scope the search with a more precise tool (AST-based, or a scoped extraction like the awk block extraction used in M-0027 Guard 9) or accept that the invariant is not expressible as a simple grep and rely on build/test coverage instead.
...
- **Trust completed-milestone "N/N passing" counts** from M-0025 (now 20/20) and M-0026 (now 11/11) as of this fix, but not retroactively — the earlier tracking-doc counts pre-date the fail-fast check.
