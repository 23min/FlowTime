...
A workaround exists: rapid batch ingestion via E-0015's batch pipeline (poll every 5 minutes). This gives a useful but degraded crystal ball. True real-time prediction needs the streaming epic to be real.

### When to revisit

After E-0015's first dataset path proves batch ingestion works. At that point, the streaming note should be promoted to an epic with milestones.
