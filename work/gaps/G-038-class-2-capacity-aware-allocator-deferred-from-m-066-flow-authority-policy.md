---
id: G-038
title: Class-2 capacity-aware allocator (deferred from M-066 flow-authority policy)
status: open
discovered_in: M-066
---
...
M-066 established the three-class flow-authority policy (see [`docs/architecture/flow-authority-policy.md`](../../docs/architecture/flow-authority-policy.md) and the M-066 spec). The policy names three places flow can be authoritatively determined:
...
Class 2 was deferred from M-066 because:

- Its engineering footprint is the largest of the three (the M-066 footprint analysis estimated 150–300 LOC across schema, compile, and analyse, plus a semantic asterisk on flow-purity guarantees).
- M-066's scope was decision + policy + doc-sweep, not allocator implementation.
...
When a future milestone (likely under E-25 — Engine Truth Gate — or a successor epic) takes class-2 on, this gap is the entry point. The carrier milestone will need to:
...
- ADR-NNNN flow-authority policy — forthcoming under M-066/AC-5.
