...
The `/ultrareview` sweep against `main` for the E-0021 epic branch surfaced four low-severity issues — one pre-existing path-traversal pattern in the Engine API and three nit-level UX/correctness defects on the new `/analysis` and topology surfaces. None block the milestone wrap; all are worth tracking so they do not decay into tolerated coexistence.
...
- Finding 1 → E-0019 follow-up or a standalone patch alongside the deferred `POST /v1/run`/`POST /v1/graph` retirement; should close the whole class (all 5 call sites), not patch a single endpoint.
- Findings 2–4 → M-0044 polish milestone, or a patch on the epic branch before epic wrap if priorities permit.
