...
Surfaced during M-0050 (D-m-E24-02-03, 2026-04-25). `src/FlowTime.Sim.Core/Templates/Template.cs` carries three `Legacy*` properties on the authoring-time `Template` types, each paired with a `ShouldSerialize*` shim:
...
### Why deferred from M-0050

`Template` is authoring-time, pre-substitution. E-0024's scope is the post-substitution model boundary (the unified `ModelDto`). Touching `Template` would scope-creep the milestone.
