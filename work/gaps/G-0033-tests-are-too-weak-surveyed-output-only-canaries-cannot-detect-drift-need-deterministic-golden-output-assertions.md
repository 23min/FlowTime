...
- M-0044 AC1 real-bytes fixture (`FLOWTIME_E2E_TEST_RUNS=1`) — pinned wire-format round-trip on `state_window` warnings.
...
**Open.** Worth planning into a near-term milestone. Strongly suggest scoping into an explicit testing-rigor milestone before further engine evolution (E-0022 Time Machine: Model Fit + Chunked Evaluation, E-0015 Telemetry Ingestion, future Cloud Deployment work). Each of those will introduce more compilation paths, more analytical surfaces, more places where silent drift can hide.
...
2. **Generate the expected output bundle** by running the engine **once** at a sanctioned point in time (post-E-0024, post-E-0023, post-E-0021 wrap is a reasonable baseline). Capture: full per-series per-bin numeric table (or tight-tolerance representation), full warning set, manifest summary fields.
...
- Plan this milestone into the near-term sequence — preferably before E-0022 (Model Fit) starts, since model-fit work will compare engine output against telemetry and silently-drifting output would corrupt fit results without warning.
