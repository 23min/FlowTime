...
Open question. Not scheduled. Worth deciding before M-0046 scope lands so the implementation substrate is settled.
...
3. Re-evaluate with data. If Rust wins decisively on memory + deployment profile and the async-Rust complexity for WS fan-out is tractable, Studio (E-0023) is the natural place to land the shift — it is mostly new scope anyway.
