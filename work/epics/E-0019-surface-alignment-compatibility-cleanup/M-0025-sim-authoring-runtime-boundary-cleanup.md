...
M-0024 published the supported-surface matrix in [docs/architecture/supported-surfaces.md](../../../docs/architecture/supported-surfaces.md) and locked retention/deletion decisions A1–A6 in the milestone spec. No code changed in M-0024 — every deletion was assigned an owning downstream milestone and a grep guard. This milestone is the first deletion pass and executes every row whose `Owning milestone` column is `m-E19-02`.

Scope boundaries inherited from M-0024:

- `FlowTime.Core`, `FlowTime.Generator`, `FlowTime.API`, and `FlowTime.Sim.*` are **not renamed** and their high-level responsibilities do not change in E-0019. Generator stays frozen; its Path B extraction belongs to E-0018 m-E18-01a.
- The canonical run directory under `data/runs/<runId>/` is unchanged.
- Analytical surfaces purified by E-0016 are out of scope.
- Blazor stale-wrapper cleanup (beyond the catalog selector, which is coupled to A5) belongs to M-0027.
- Schema, template, example, and current-doc cleanup belong to M-0026.
- `FlowTime.TimeMachine` is not introduced here — that is E-0018 m-E18-01a.

The default execution path for first-party UIs during and after this milestone remains `POST /api/v1/orchestration/runs` on `FlowTime.Sim.Service` per A1. Sunsetting that endpoint is an E-0018 decision, not this milestone's.
...
- Documentation for this endpoint (in `docs/reference/contracts.md` and elsewhere) is updated by M-0026 — this milestone only removes the code branch.
...
- `POST /v1/run` and `POST /v1/graph` deletion is deferred out of M-0025. Retained as a transitional test-infrastructure surface until the Provenance/Parity/Legacy test suites are migrated to an alternative run-creation path (either a test-only in-process adapter over `Graph.Evaluate` / `RunOrchestrationService` or the supported Sim orchestration endpoint with template fixtures). A decisions.md entry (D-0042) records the scope change and the reason.
...
- `FlowTime.Generator` — frozen in E-0019; any Generator work belongs to E-0018 m-E18-01a.
...
- Blazor stale-wrapper cleanup beyond `CatalogPicker.razor` — that is M-0027's job.
- Schema files under `docs/schemas/` — M-0026 owns any deprecated-schema removal.
- Template files under `templates/` — M-0026 owns any deprecated-template removal.
- Example files under `examples/` — M-0026 owns schema-compatibility example retirement.
...
- Schema, template, example, or docs cleanup (M-0026 owns those).
- Blazor stale compatibility wrappers outside the catalog picker (M-0027).
- Replacing the deleted validation endpoint with a tiered validation API on Sim — that is explicitly an E-0018 m-E18-01b deliverable per A6.
...
- **DO NOT** widen the scope into schema/template/example cleanup. Those are M-0026.
- **DO NOT** touch the canonical run directory layout at `data/runs/<runId>/`. The bundle archive layer is separate.
- **DO NOT** re-home `TemplateInvariantAnalyzer` into `FlowTime.Core` in this milestone. That is an E-0018 m-E18-01b concern.
