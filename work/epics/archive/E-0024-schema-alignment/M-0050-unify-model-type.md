...
This milestone implements those decisions. It is the single milestone where code shape changes — schema (M-0051), validator (M-0052), and canary (M-0053) follow what this milestone builds.

## Acceptance criteria

### AC-1 — Unified type exists at its ratified home

**Unified type exists at its ratified home.** The type named in M-0049 lives at its ratified namespace and name. A reader who opens `POST /v1/run`'s handler can follow the type reference to a single definition that represents the full post-substitution model.
...
**Satellite Sim-side types are deleted.** `SimNode`, `SimOutput`, `SimProvenance`, `SimTraffic`, `SimArrival`, `SimArrivalPattern` are removed from the repo. Each satellite either merged into the unified type's equivalent (per M-0049) or was deleted because it had no consumer. `grep -rn "SimNode\b\|SimOutput\b\|SimProvenance\b\|SimTraffic\b\|SimArrival\b\|SimArrivalPattern\b" --include='*.cs'` returns zero hits.
### AC-6 — Leaked-state fields dropped from emission

**Leaked-state fields dropped from emission.** Per M-0049's decisions, `window`, `generator`, top-level `metadata`, and top-level `mode` no longer appear in emitted YAML. Whatever traceability content was meaningful has been moved into `provenance`.
...
- **Emission stays declarative.** YamlDotNet serialization attributes (`[YamlMember]`, `[YamlIgnore]`) plus `DefaultValuesHandling.OmitNull | OmitEmptyCollections` configured on the `SerializerBuilder` are the mechanism for omitting leaked-state and absent fields — not custom string manipulation. The emitter's behavior is inspectable from the type definition. **Note:** YamlDotNet does not honor a `ShouldSerialize{X}()` convention — that is a Json.NET / `XmlSerializer` pattern. Per D-m-E24-02-03, all `ShouldSerialize*` shims that survive into M-0050 either die with their host type (`SimNode`, `LegacyStart`) or are replaced by nullable-property + `OmitNull` semantics.
- **No Sim→Engine dependency inversion.** The unified type lives at its M-0049 home. If that home is `FlowTime.Contracts`, both Sim and the Engine reference it — Sim already references `FlowTime.Contracts`, and the Engine likewise. If the home is `FlowTime.Core`, Sim adds the reference. The Engine never references Sim.

## Design Notes

- **Order of work:**
  1. Introduce the unified type at its ratified home with all fields from M-0049's final list.
...
- **Emitter attributes.** YamlDotNet's camelCase naming convention (`CamelCaseNamingConvention.Instance`) is already the project default. `DefaultValuesHandling.OmitNull | OmitDefaults` on optional fields ensures that fields like `nodes[].source` (if M-0049 decided "drop from emission when absent") do not appear in serialized output when unset. `ShouldSerialize{PropertyName}()` is available for empty-collection guards (e.g. omit `classes: []`).
- **Provenance.** Per M-0049, the `provenance` block is declared as a nested type on the unified model (or as a property on a flat unified model — the shape is M-0049's decision). Either way, its camelCase keys match `SimProvenance`'s existing emission and `ProvenanceMetadata`'s existing reader.
- **`nodes[].metadata`.** Stays as a load-bearing `Dictionary<string, string>` per M-0049's keep decision. The unified node type exposes it as a declared property with the schema-description consumers (`GraphService`, `StateQueryService`, `RunArtifactWriter`).
...
- The unified type's source file (new; location per M-0049 — likely `src/FlowTime.Contracts/Models/` or extending `src/FlowTime.Core/Models/ModelDefinition.cs`)
...
- Sim satellite types: `SimNode.cs`, `SimOutput.cs`, `SimProvenance.cs`, `SimTraffic.cs`, `SimArrival.cs`, `SimArrivalPattern.cs` (deleted per M-0049 disposition)
- `src/FlowTime.Core/Models/ModelParser.cs` (switched to unified type)
- `src/FlowTime.Core/Models/ModelDefinition.cs` (becomes the unified type or is replaced — per M-0049)
...
- `ModelValidator` deletion — stays with E-0023 M-0048.

## Dependencies

- M-0049 landed. Unified type home, name, provenance shape, field dispositions, and satellite-type dispositions are all named in M-0049's tracking doc before this milestone starts.
