...
Spec success criterion from E-0018:
```
...
- M-0003 (TimeMachineValidator) — delivered
- M-0006 (`IModelEvaluator`, `SweepRunner`, `ConstNodePatcher`) — delivered
- M-0007 (`SensitivityRunner`, `ConstNodeReader`) — delivered
- M-0008 (`GoalSeeker`) — delivered
- M-0009 (`Optimizer`, `OptimizeSpec`) — delivered
- M-0010 (`SessionModelEvaluator`, evaluator config switch) — delivered
...
- **Test isolation.** Integration tests spawn the Rust engine subprocess per test. Same
  skip-if-missing pattern as M-0010 integration tests.
