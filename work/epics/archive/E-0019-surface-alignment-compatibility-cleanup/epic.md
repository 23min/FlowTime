---
id: E-0019
title: Surface Alignment & Compatibility Cleanup
status: done
---
...
E-0016 purifies analytical truth and contract facts, but broader repo surfaces still carry compatibility debt that sits outside the formula-first boundary:
...
These are not analytical truth seams, so they should not be folded back into E-0016. But they also should not remain unowned. If left alone, temporary compatibility layers become a de facto product support promise and the parallel UI surfaces will drift.

This epic creates an explicit post-purification cleanup lane: once E-0016 finishes, FlowTime should tighten the rest of its first-party surfaces around one current contract and a small set of explicitly supported user paths. That includes keeping Blazor current as a supported debugging/plan-B surface, not retiring it.

## Scope

### In Scope

- Inventory of first-party compatibility and legacy seams outside E-0016's analytical boundary
- Explicit supported-surface policy across Engine, Sim, Svelte UI, Blazor UI, docs, schemas, and examples
- Explicit boundary between current Sim authoring/orchestration surfaces and the future E-0018 Time Machine foundation
...
- Analytical truth, emitted-series logic, by-class purity, warning fact ownership, or consumer fact publication already owned by E-0016
- New analytical primitives from E-0010 Phase 3 (`p3d`, `p3c`, `p3b`)
- Delivering missing Svelte UI product features themselves; E-0011 still owns additive Svelte buildout
- Blazor UI retirement or functionality removal as a cleanup goal
- The new Time Machine runtime parameter foundation, reevaluation API, evaluation SDK, CLI/sidecar, or optimization/fitting modes owned by E-0018
- Chunked/stateful execution semantics and streaming/stateful seams owned by later E-0018 layers or dedicated streaming work
...
- Starts after E-0016 establishes purified current contracts; runtime/API cleanup milestones should consume those contracts rather than redefine them
- Does not block E-0010 Phase 3 resume by default; this runs as a parallel post-purification cleanup lane unless a specific milestone intentionally touches a shared contract gate
...
E-0019 and E-0018 are adjacent but they do different work:

- **E-0019** narrows or deletes the current first-party residue around Sim authoring/orchestration, storage-backed drafts, archived bundle refs, runtime catalogs, and ambiguous caller paths. It decides what remains supported on today's surfaces.
- **E-0018** builds the replacement Time Machine foundation: runtime parameter identity, deterministic overrides, reevaluation APIs, evaluation SDK, and CLI/sidecar over compiled graphs.

The current Sim orchestration path is therefore not the default path forward for Time Machine evaluation. If an E-0019 deletion depends on new capability, that dependency should be made explicit against E-0018 rather than preserving the current residue indefinitely.

## Current Findings To Use

| Surface / Residue | Current Role | Risk If Left Unowned | E-0019 Intent |
|------------------|--------------|----------------------|-------------|
...
- [x] First-party clients no longer maintain duplicate endpoint, metrics, or health fallback logic where the canonical contract exists (M-0027: stale `IFlowTimeSimApiClient.RunAsync`/`GetIndexAsync`/`GetSeriesAsync` deleted and callers rewired to supported `CreateRunAsync` + Engine query surface)
...
- [x] Active UI/template surfaces no longer generate or promote deprecated schema shapes such as `binMinutes`-based demo templates (M-0026: Blazor mock template service, sample fixture, CLI verbose label, and UI test keys all rewritten to `binSize`/`binUnit`)
- [x] Legacy examples, docs, and schema references are either archived/historical or deleted; current docs present one canonical surface (M-0026: schema-migration example YAMLs moved to `examples/archive/`; stale `docs/ui/template-integration-spec.md` moved to `docs/archive/ui/`; active docs cleaned)
...
- [x] E-0018 planning remains clean: no current Sim draft/catalog/bundle choreography is treated as the default programmable/Time Machine contract
- [x] Grep and regression audits prove targeted legacy/fallback helpers are removed or isolated to historical/archive surfaces only (M-0025: 21 guards; M-0026: 11 guards; M-0027: 11 guards, all 43 passing)
...
This epic starts immediately after E-0016 as a post-purification cleanup lane. It should run in parallel with resumed E-0010 Phase 3 work and final E-0011 parity work, not silently replace them.

- E-0016 removes analytical truth and consumer-fact reconstruction seams.
- E-0019 removes the remaining non-analytical legacy, fallback, and current-surface debt around those purified contracts.
- E-0011 continues as a parallel UI track; E-0019 keeps both first-party UIs aligned to the same current Engine/Sim contracts.

## Milestones

| ID | Title | Summary | Depends On | Status |
|----|-------|---------|------------|--------|
| M-0024 | Supported Surface Inventory, Boundary ADR & Exit Criteria | Inventory compatibility seams, define supported vs historical surfaces, publish the terminology/ownership ADR, and pin retention/deletion gates for drafts, bundles, catalogs, and import paths. | E-0016 | completed |
| M-0025 | Sim Authoring & Runtime Boundary Cleanup | Narrow Sim to explicitly supported authoring/orchestration surfaces, remove transitional catalog/runtime callers, and keep Engine import/query ownership explicit. | M-0024 | completed |
| M-0026 | Schema, Template & Example Retirement | Remove or archive deprecated schema, demo template, and example material from active current surfaces. | M-0024 | completed |
| M-0027 | Blazor Support Alignment | Remove stale `FlowTime.UI` compatibility wrappers, keep Blazor aligned with current Engine/Sim contracts, and define clear supported debugging/operator workflows alongside the parallel Svelte UI. | M-0025, M-0026 | completed |
...
- stale metrics/state reconstruction in first-party Blazor consumers that survive E-0016
