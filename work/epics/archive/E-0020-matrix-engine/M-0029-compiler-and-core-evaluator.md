...
M-0028 delivered the Rust workspace, model types with YAML deserialization, and the expression parser. The crate can parse any model and any expression, but cannot compile or evaluate anything.
...
- **No topology:** This milestone handles flat node lists only. Topology synthesis (serviceWithBuffer queue nodes) comes in M-0030.
- **PMF nodes:** `kind: "pmf"` computes a constant expected value from the distribution. Can be included here as a simple op, or deferred to M-0030. Include if straightforward.

## Out of Scope

- Topology synthesis (queue nodes, retry echo) — M-0030
- Sequential ops (QueueRecurrence, Shift, Convolve, DispatchGate) — M-0030
- Routing and constraints — M-0031
- Derived metrics — M-0032
- Artifact writing (CSVs, JSON) — M-0033
- SHIFT/feedback handling — M-0030

## Dependencies

- M-0028 complete (model types + expression parser)
