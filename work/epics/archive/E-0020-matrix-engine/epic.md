...
> **Status note:** M-0028–10 all complete
...
- **Feedback subgraphs require special handling**: the bin-by-bin evaluator added in E-0010 p3b is correct but is a bridge to the matrix model where bin-sequential evaluation is the default.
...
- Compiles to native CLI binary (zero runtime, ~5-10 MB) and to WebAssembly (engine in browser for E-0017).
...
E-0017 (Interactive What-If) needs incremental re-evaluation (<50ms response). E-0018 (Time Machine) needs parameter sweeps (1 compile + N partial replays). Both are economically different with the matrix model vs without it. Building E-0017/E-0018 on the object graph means building them twice.

The bin-by-bin feedback evaluator from E-0010 p3b validates the approach — it IS the matrix model scoped to feedback subgraphs.
...
- E-0017 interactive features (parameter sessions, push channel)
- E-0018 advanced modes (sweep, optimize, fit) — though the plan structure enables them
...
- E-0010 complete (provides the analytical primitives the Rust engine must reproduce)
- E-0016 complete (provides the pure evaluation model the Rust engine implements)
...
|----|-------|---------|--------|
| M-0028 | Scaffold, types, and parsers | Rust crate, model types with serde, YAML deserialization, expression parser. Reference model fixtures. Devcontainer Rust toolchain. | complete |
| M-0029 | Compiler and core evaluator | Column map, topo sort, plan generation for const/expr nodes. Evaluator loop + element-wise ops. First end-to-end parity on simple models. | complete |
| M-0030 | Topology and sequential ops | Queue synthesis, QueueRecurrence, Shift, Convolve, DispatchGate, PMF, WIP limits with overflow. Feedback subgraphs (bin-sequential). | complete |
| M-0031 | Routing and constraints | Router flow materialization, constraint allocation, multi-class flow distribution — all as plan ops. | complete |
| M-0032 | Derived metrics and analysis | Utilization, latency, cycle time, Cv, Kingman as plan ops. Invariant analysis as column arithmetic. Warnings. | complete |
| M-0033 | Artifacts, CLI, and integration | CSV/JSON artifact writer. CLI (eval, validate, plan). | complete |
| M-0034 | .NET subprocess bridge | SHA256 hashing + manifest.json. RustEngineRunner subprocess bridge. Config switch. Parity tests. | complete |
| M-0035 | Full parity harness | All 21 fixtures tested against C# engine. `outputs:` filtering. Green/red parity matrix. | complete |
| M-0036 | Per-class decomposition and edge series | Per-class columns, edge metrics, class assignment. Engine core feature-complete. | complete |
| M-0037 | Artifact sink parity | Full directory layout. StateQueryService compatible. RunArtifactWriter replaceable. | complete |
...
- After **M10**: artifact sink produces full layout — C# RunArtifactWriter replaceable, E-0017/E-0018 unblocked
