...
After m-E17-05, both nodes and edges in the topology graph are colored by the **mean** of their series across all bins. The mean is a useful summary, but it collapses time — users cannot see:
...
The m-E17-03 spec explicitly noted this as a future enhancement:
...
5. **AC-5: `buildEdgeMetricMap` accepts optional bin.** The same optional `bin` parameter is added to `buildEdgeMetricMap` (from m-E17-05), with identical semantics.
