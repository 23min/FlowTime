...
acs:
  - id: AC-1
    title: Topology keyboard navigation + ARIA structure (a11y parity with
    status: met
  - id: AC-2
    title: Topology node selection stroke rule (closes one-way cross-link for
    status: met
  - id: AC-3
    title: Bidirectional edge cross-link via new selectedEdge field
    status: met
  - id: AC-4
    title: Dark-mode audit across all E-0021 surfaces
    status: met
  - id: AC-5
    title: Loading skeletons on workbench + analysis surfaces
    status: met
  - id: AC-6
    title: Transitions rule applied across the workbench
    status: met
  - id: AC-7
    title: Elevation audit + token normalization
    status: met
  - id: AC-8
    title: validation-panel.svelte:157 cosmetic collapse
    status: met
  - id: AC-9
    title: '"Indicators absent on heatmap" Playwright assertion'
    status: met
  - id: AC-10
...
Close E-0021 with a polish pass over the workbench and analysis surfaces: bring topology to the heatmap's accessibility bar (keyboard + ARIA), complete the bidirectional cross-link the M-0043 / M-0044 milestones started (node *and* edge selection now flows symmetrically between the panel, the cards, the topology DAG, and the heatmap), audit dark-mode + elevation tokens across every E-0021 surface, fill the empty→populated flicker on `/time-travel/topology` and `/analysis` with skeletons, settle the transitions rule, and clean up two cheap follow-ups left in place by M-0044. The milestone is the visual-polish + a11y gate before E-0021 wraps; it ships no new analytical capability.

## Context

### What landed in the previous E-0021 milestones

- **Workbench paradigm (M-0038 / M-0039):** density tokens, dag-map click/hover events, click-to-pin node + edge cards, metric chip bar, class filter, dark/light theme.
- **Analysis surfaces (M-0040 → M-0042):** `/analysis` route with sweep / sensitivity / goal-seek / optimize tabs; shared `AnalysisResultCard` + `ConvergenceChart`.
- **Heatmap view (M-0043):** typed `<ViewSwitcher>`, shared view-state store (`view-state.svelte.ts`) with `selectedCell`, full-window 99p-clipped color scale, shared node-mode toggle, `--ft-pin` / `--ft-highlight` chrome tokens.
- **Validation surface (M-0044):** widened `StateWarning` type, validation panel as left column inside the workbench panel, topology node + edge warning indicators, workbench-card warning dot + severity row tinting, validation-row click → pin + cross-highlight, FLIP card animations, `--ft-warn` / `--ft-err` / `--ft-info` chrome tokens.
...
- **Transitions audit.** M-0044 added FLIP animations on workbench card insert/remove (`animate:flip` at `+page.svelte:761,782`). The audit produces a written rule for what gets motion (cards, result swaps), what stays instant (view-switcher, context changes), and applies the rule consistently to the analysis-tab result swap and the run-selector dropdown content swap.
...
- **`validation-panel.svelte:157` no-op terminal collapse.** `{#if row.kind === 'node'}{row.key}{:else}{row.key}{/if}` is a no-op left in by an in-flight refactor; collapse to `{row.key}`. Flagged in M-0044 wrap audit.
...
- Click a validation panel row with `nodeId` (existing path from M-0044) → node strokes turquoise on the topology DAG.
### AC-3 — Bidirectional edge cross-link via new selectedEdge field
...
- **Validation panel edge rows wire `setSelectedEdge`.** When the user clicks a validation row whose identity is an edge (the `edgeWarnings`-keyed rows from M-0044 AC9), the existing pin path is preserved *and* `viewState.setSelectedEdge(from, to)` is called — symmetric to the node path that already calls `setSelectedCell`. Edge rows whose edge id does not parse as `from→to` (rare; mismatched analyser output) preserve today's pin-only behaviour.
- **Selection-clear on unpin.** When the selected edge is unpinned (via the edge card's ✕ button or any other unpin path), `clearSelectedEdge()` fires automatically — same shape as the M-0043 `unpinAndClearSelection` helper for nodes.
### AC-4 — Dark-mode audit across all E-0021 surfaces

**Dark-mode audit across all E-0021 surfaces.** Run a single sweep over the workbench, analysis, validation, and chrome surfaces in dark mode looking for token-resolution bugs, contrast issues, and breakage of M-0044's transparent severity-row backgrounds. Surfaces in scope:
...
### AC-5 — Loading skeletons on workbench + analysis surfaces
...
### AC-6 — Transitions rule applied across the workbench
...
- **Card insert / remove:** 220 ms FLIP + 160 ms fade-in / 120 ms fade-out (already shipped in M-0044; ratify here).
...
- **Validation row highlight on selection (M-0044 AC10 cross-link):** **no transition** (selection feedback should be instant).
...
### AC-7 — Elevation audit + token normalization

**Elevation audit + token normalization.** Single pass over the shadow / border / layering tokens used across the E-0021 surfaces. Catalogue the current uses (border-only, `shadow-sm`, `shadow-md`, custom `box-shadow`, ring-1, …) in the tracking doc; pick a small canonical set (recommend: flat / `shadow-sm` for cards / `shadow-md` for popovers and dropdowns / no shadow on the topology canvas); apply consistently. Out of scope: introducing new elevation tokens; this is a normalization-to-existing pass.
### AC-8 — validation-panel.svelte:157 cosmetic collapse

**`validation-panel.svelte:157` cosmetic collapse.** Replace `{#if row.kind === 'node'}{row.key}{:else}{row.key}{/if}` with `{row.key}`. Verify no behavioural change via the existing M-0044 vitest tests on the panel.
### AC-9 — "Indicators absent on heatmap" Playwright assertion

**"Indicators absent on heatmap" Playwright assertion.** Add an explicit assertion to `tests/ui/specs/svelte-validation.spec.ts` (or a sibling spec) confirming that when `viewState.activeView === 'heatmap'`, no `[data-warning-indicator]` elements are present in the canvas region. Closes the contract guard the M-0044 wrap audit identified as covered indirectly today.
### AC-10 — Testing — Playwright + vitest, every reachable branch covered
...
**Playwright** (extending or siblinging existing E-0021 specs; graceful-skip on dev-server / API unavailability):
...
A line-by-line branch audit of the new UI + helpers against tests is recorded in the tracking doc's Coverage Notes section, matching the M-0042 / M-0043 / M-0044 audit structure.
## Constraints
...
- **Color-blind validation + pattern encoding deferred.** Per user decision 2026-04-28; remains an open gap in `work/gaps.md` and a candidate for a follow-up after E-0021 wraps.
...
- **`.node-selected` stroke style.** `stroke: var(--ft-highlight); stroke-width: 2; fill-opacity: 1;` — same hue as the workbench-card title cross-highlight, mirrors the M-0043 convention. Avoid changing the node fill; the metric color stays the encoding signal, the stroke is the chrome signal.
...
- **Edge identity normalization.** `selectedEdge` uses the `{from, to}` shape (matches the workbench's `pinnedEdges` shape and avoids the string-key parsing edge cases). The `edgeWarnings` map keys remain string-form (`from→to`), and the validation row → `setSelectedEdge` path normalizes by parsing on the arrow boundary (already done in M-0044 at `+page.svelte:262-265`).
...
- **Mobile / responsive layout audit.** Out of E-0021 scope per epic spec.
- **Heatmap-row warning badges.** M-0044 follow-up; not this milestone.
- **Decomposition / comparison / flow-balance views.** Not E-0021 scope.
- **Editor-time validation surface.** Not E-0021 scope; expert-authoring epic.

## Dependencies

- **M-0043 Heatmap View** — provides the shared view-state store (`view-state.svelte.ts`) that gains the `selectedEdge` field per AC3, the `selectedCell` field that AC2 reads, and the chrome-token convention (`--ft-pin`, `--ft-highlight`) that AC2 reuses.
- **M-0044 Validation Surface** — provides the validation panel, the severity-row click path that AC3 extends with `setSelectedEdge` for edge rows, the `--ft-warn` / `--ft-err` / `--ft-info` chrome tokens that AC4 audits, and the workbench-card severity dot + FLIP card animation that AC6 ratifies.
