---
id: E-0021
title: Svelte Workbench & Analysis Surfaces
status: done
---
...
The fork decision (2026-04-15) declared Svelte the platform for all new surfaces and Blazor maintenance-only. E-0011 delivered M1-M4 + M6: scaffold, API layer, topology via dag-map, timeline with heatmap scrubbing, and run orchestration. E-0017 added the what-if page with live parameter manipulation (200 vitest + 26 Playwright).
...
- What-if page (E-0017 — WebSocket parameter panel with real-time topology updates)
...
- Analysis surfaces for Time Machine APIs (sweep, sensitivity, goal-seek, optimize, validate — all built in E-0018 but with no UI)
...
The UI paradigm proposal (`docs/explorations/ui-workbench/reference/ui-paradigm.md`) established the direction: topology as navigation surface (structure + one color dimension), workbench panel for depth (click-to-pin cards), and layered views (heatmap, decomposition, comparison). E-0021 implements that paradigm and adds the first Time Machine analysis surfaces.

### Supersedes

- **E-0011 M5** (Inspector & Feature Bar) — evolves into M-0038/02 workbench paradigm
- **E-0011 M7** (Dashboard) — deferred; workbench + heatmap cover the same ground better
- **E-0011 M8** (Polish) — absorbed into M-0045

E-0011 remains paused at M6 as a completed historical track.

### Relationship to other unplanned epics

- **UI Workbench & Topology Refinement** (`docs/explorations/ui-workbench/spec.md`) — M-0038 and M-0039 implement its goals (G1-G5)
- **UI Analytical Views** (`docs/explorations/ui-analytical-views/spec.md`) — M-0043 delivers the heatmap view; decomposition and comparison views are future extensions
- **Expert Authoring Surface** (`docs/explorations/expert-authoring-surface/spec.md`) — not in E-0021 scope; depends on E-0021's workbench and validation surface being in place
...
- E-0015 telemetry ingestion UI (parallel track, not E-0021 scope)
...
| Audience | Surface | Relationship to E-0021 |
|----------|---------|---------------------|
...
- .NET 9 backend APIs are the source of truth. Svelte UI is primarily a consumer. Two explicit carve-outs are admitted: (1) per **D-0046**, E-0021 may add **read-only run-adjacent endpoints** that strictly serve already-persisted run artifacts (e.g. `GET /v1/runs/{runId}/model`); (2) per **D-0047**, E-0021 may extend existing compute-endpoint response shapes with **additive** fields that expose state the endpoint already computes internally (specifically the `trace` field on `/v1/goal-seek` and `/v1/optimize`). Authoring, orchestration, new compute endpoints, write-path endpoints, and non-additive shape changes to existing compute endpoints remain out of scope and need their own decision record if ever proposed.
- dag-map enhancements must remain general-purpose (not FlowTime-specific API)
- Existing what-if page (E-0017) must continue working after workbench changes
- Existing run orchestration (E-0011 M6) must continue working
...
| Analysis API calls are slow for large sweeps (session evaluator helps) | Med | Show progress/loading state. `SessionModelEvaluator` (M-0010) is the default and is fast for moderate sweeps. |
...
|----|-------|---------|--------|
| M-0038 | Workbench Foundation | Density system, dag-map events (library), topology as navigation (one color dimension), workbench panel with click-to-pin node cards | **complete** (merged 2026-04-17) |
| M-0039 | Metric Selector & Edge Cards | Metric chip bar, edge click-to-pin, edge cards, class filter | **complete** (merged 2026-04-17) |
| M-0040 | Sweep & Sensitivity Surfaces | `/analysis` route with tabs, sweep config + results, sensitivity bar chart | **complete** (merged 2026-04-17; ultrareview follow-ups 2026-04-20) |
| M-0041 | Goal Seek Surface | Goal-seek panel, shared convergence chart + result card, `trace` on `/v1/goal-seek` and `/v1/optimize` (per D-0047) | **complete** (2026-04-22) |
| M-0042 | Optimize Surface | N-param Nelder-Mead surface reusing shared convergence chart + result card from M-0041; per-param range table | **complete** (2026-04-22) |
| M-0043 | Heatmap View | Nodes-x-bins grid, row sorting, click-to-jump, view switcher (topology/heatmap), shared full-window color scale, shared node-mode toggle | **complete** (2026-04-24) |
| M-0044 | Validation Surface (Svelte) | Consumer-side type widening on `state_window` warnings; validation panel as left column inside workbench panel; topology node + edge warning indicators; workbench-card warning surfaces; bidirectional cross-link via shared view-state store; Playwright real-bytes fixture for AC1 round-trip. No backend work. | **complete** (2026-04-28) |
| M-0045 | Visual Polish & Dark Mode QA | Topology keyboard + ARIA retrofit, full bidirectional cross-link (node + edge), dark mode audit, loading skeletons, transitions rule, elevation normalization | **complete** (2026-04-28) |

## ADRs

- ADR-E21-01: New epic E-0021 rather than resuming E-0011. E-0011 was "clone Blazor in Svelte." E-0021 is "build the workbench paradigm + new surfaces." E-0011's remaining milestones (M5/M7/M8) are absorbed with different scope.
...
- ADR-E21-04: Dashboard (E-0011 M7) deferred. Workbench + heatmap view cover per-node overview better than a dedicated dashboard page.
- ADR-E21-05: Density pass is M-0038 scope, not polish. Building on spacious defaults and compressing later cascades through every component.
