...
- **Status:** **Ready to resume (2026-04-25; rescoped 2026-04-26)** — E-24 Schema Alignment closed; E-23 spirit reframed: make `model.schema.yaml` the only declarative source of structural truth and `ModelSchemaValidator` the only runtime evaluator — eliminate every "embedded schema" outside the canonical schema (`ModelValidator.cs` hand-rolled rules, parser tolerations, silent emission defaults, post-parse orchestration checks). E-24 fixed type + schema-document embedment; E-23 closes the rule-evaluator embedment. Three milestones, all ready.
