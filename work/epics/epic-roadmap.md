...
- **Status:** In-progress — m-E24-01 Inventory & Design Decisions **complete (2026-04-25, commit `c43e8c0`)**; m-E24-02 Unify Model Type **complete (2026-04-25, commits `131dd35` / `c4064bf` / `3806097` / `dece22f`)**; m-E24-03 Schema Unification **complete (2026-04-25, commit `d6a3263`)**; **m-E24-04 Parser/Validator Scalar-Style Fix in-progress (started 2026-04-25)** on branch `milestone/m-E24-04-parser-validator-scalar-style-fix`. Epic integration branch `epic/E-24-schema-alignment` (created from `main` 2026-04-24). Internal planning ratified **Option E (unify)** on 2026-04-24 per `D-2026-04-24-037`. Interrupts E-23 at m-E23-01.
...
- **Key milestones (5):** m-E24-01 Inventory & Design Decisions — **complete (2026-04-25, commit `c43e8c0`)**; doc-only; unified type home (`FlowTime.Contracts`), name (`ModelDto`), nested 7-field camelCase provenance shape, six open questions ratified (Q1–Q6 + A1–A5) · m-E24-02 Unify Model Type — **complete (2026-04-25, commits `131dd35` / `c4064bf` / `3806097` / `dece22f`)**; all 14 ACs landed; `ModelDto`+`ProvenanceDto` in `FlowTime.Contracts`; `SimModelArtifact` + 6 satellites deleted; `SimModelBuilder` rewritten; Engine intake unified; YamlDotNet 17.0.1; 1,755 tests passing · m-E24-03 Schema Unification — **complete (2026-04-25, commit `d6a3263`)**; all 14 ACs landed; schema rewritten against unified `ModelDto`; canary 676 → 231 (schema-rewrite shapes 587 → 0; remaining are m-E24-04 ParseScalar) · m-E24-04 Parser/Validator Scalar-Style Fix — **in-progress (started 2026-04-25)** on branch `milestone/m-E24-04-parser-validator-scalar-style-fix`; mirrored `ParseScalar` fix; pre-state canary 231 errors, target post-state 0 · m-E24-05 Canary Green + Hard Assertion
- **ADRs (ratified in m-E24-01):** ADR-E-24-01 Unify · ADR-E-24-02 Forward-only regeneration · ADR-E-24-03 Schema declares only consumed fields · ADR-E-24-04 (m-E24-04) `ScalarStyle.Plain` gates `ParseScalar`.
...
- **Decisions:** `work/decisions.md` → `D-2026-04-24-036` (E-23 paused, E-24 created) · `D-2026-04-24-037` (Option E ratified; 5-milestone plan)
...
- **Status:** **Paused (2026-04-24)** — m-E23-01 Schema Alignment surfaced 16 divergences beyond E-23's scope; E-24 Schema Alignment takes over the convergence work. Uncommitted m-E23-01 input material (schema edits, tracking doc, gaps entries) stashed on branch `milestone/m-E23-01-schema-alignment` as preserved input for E-24 m-E24-01. Resume when E-24 closes; at that point m-E23-02 (call-site migration) and m-E23-03 (`ModelValidator` delete) become byte-trivial.
...
- **Milestone status after pause:** m-E23-01 paused (pending E-24) · m-E23-02 pending (awaits E-24 close) · m-E23-03 pending (awaits E-24 close)
- **Dependencies:** E-24 Schema Alignment
- **Unblocks (after E-23 resumes + closes):** m-E21-07 Validation Surface (Svelte)
- **Regression canary:** `tests/FlowTime.Integration.Tests/TemplateWarningSurveyTests.cs` — committed to `main` (commit `1234814`); promotes to hard assertion at m-E24-05 close
