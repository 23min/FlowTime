...
- **Folder:** `work/epics/E-24-schema-alignment/`
- **Status:** **Complete (2026-04-25)** — all five milestones landed on `epic/E-24-schema-alignment`. m-E24-01 Inventory & Design Decisions **complete (commit `c43e8c0`)**; m-E24-02 Unify Model Type **complete (commits `131dd35` / `c4064bf` / `3806097` / `dece22f`)**; m-E24-03 Schema Unification **complete (commit `d6a3263`)**; m-E24-04 Parser/Validator Scalar-Style Fix **complete (commit `a7c984f`)** — canary 231 → 0 errors; m-E24-05 Canary Green + Hard Assertion **complete** — hard `val-err == 0` build-time gate with regression-catching verified end-to-end. Integration branch ready for merge to `main` and subsequent archive to `work/epics/completed/E-24-schema-alignment/`. Internal planning ratified **Option E (unify)** on 2026-04-24 per `D-2026-04-24-037`. Closure logged in `D-2026-04-25-038`.
- **Goal:** Unify FlowTime's post-substitution model representation. One C# type. One YAML schema. One validator. `SimModelArtifact` **deleted**; Sim builds the unified type directly; Engine parses it directly. `Template` (authoring-time) stays distinct. Forward-only — no bundle migration. camelCase throughout. Promote `TemplateWarningSurveyTests` to a hard-asserting regression guard (`val-err=0` across all twelve templates at `ValidationTier.Analyse`).
- **Why now:** The two-type split is accidental drift, not a designed interface. `SimModelArtifact` was introduced in commit `ce9ec9e` (Oct 2025) shaped to the template schema rather than to `model.schema.yaml` which predated it; no C# type boundary exists (only a YAML string that fails the Engine's own schema in every shipped template); m-E23-01's canary survey reported 726 validator errors across 16 divergence shapes. Investigation agent: `a5aa3dfe26394aff5`; survey agent: `a07d52c12dcaf3538`.
- **Option A (rejected):** Preserve both types with a projection layer. Rejected as preserving accidental drift.
- **Key milestones (5):** m-E24-01 Inventory & Design Decisions — **complete (2026-04-25, commit `c43e8c0`)**; doc-only; unified type home (`FlowTime.Contracts`), name (`ModelDto`), nested 7-field camelCase provenance shape, six open questions ratified (Q1–Q6 + A1–A5) · m-E24-02 Unify Model Type — **complete (2026-04-25, commits `131dd35` / `c4064bf` / `3806097` / `dece22f`)**; all 14 ACs landed; `ModelDto`+`ProvenanceDto` in `FlowTime.Contracts`; `SimModelArtifact` + 6 satellites deleted; `SimModelBuilder` rewritten; Engine intake unified; YamlDotNet 17.0.1; 1,755 tests passing · m-E24-03 Schema Unification — **complete (2026-04-25, commit `d6a3263`)**; all 14 ACs landed; schema rewritten against unified `ModelDto`; canary 676 → 231 (schema-rewrite shapes 587 → 0; remaining are m-E24-04 ParseScalar) · m-E24-04 Parser/Validator Scalar-Style Fix — **complete (2026-04-25, commit `a7c984f`)**; all 9 ACs landed; mirrored `ParseScalar` fix + sibling `QuotedAmbiguousStringEmitter` for round-trip symmetry (ADR-E-24-05); canary **231 → 0 errors**; +64 new tests · m-E24-05 Canary Green + Hard Assertion — **complete (2026-04-25)**; all 8 ACs landed; `Survey_Templates_For_Warnings` rewritten with phase-1 hard `val-err == 0` assertion + phase-2 graceful-skip live-API survey; regression-catching verified by transient `QuotedAmbiguousStringEmitter` un-wire (canary fails at 231 errors, recovers on restore); full `.NET` suite green; AC4 grep audits clean; `docs/schemas/model.schema.md` provenance section rewritten to canonical 7-field nested camelCase form; E-23 status flipped to ready-to-resume; `D-2026-04-25-038` logs E-24 close
...
- **Dependencies:** none upstream; pauses E-23 at m-E23-01
- **Unblocks:** E-23 resume (m-E23-02 + m-E23-03 become byte-trivial); m-E21-07 Validation Surface eventually; E-15 Telemetry Ingestion `nodes[].source` forward contract
- **Decisions:** `work/decisions.md` → `D-2026-04-24-036` (E-23 paused, E-24 created) · `D-2026-04-24-037` (Option E ratified; 5-milestone plan) · `D-2026-04-25-038` (E-24 closed; E-23 ready to resume)
- **Reference:** `work/epics/E-24-schema-alignment/spec.md`

#### E-23 — Model Validation Consolidation

- **Folder:** `work/epics/E-23-model-validation-consolidation/`
- **Status:** **Ready to resume (2026-04-25)** — E-24 Schema Alignment closed (all five milestones landed on `epic/E-24-schema-alignment`); m-E23-02 (call-site migration) and m-E23-03 (`ModelValidator` delete) are now byte-trivial mechanical cleanup because `ModelSchemaValidator`, the schema, Sim's emitter, and the Engine's reader literally share a single unified type definition (`ModelDto` / `ProvenanceDto` in `FlowTime.Contracts`). Reentry needs a status-reconciliation pass on the `milestone/m-E23-01-schema-alignment` branch's stashed input material (most input has been absorbed by E-24).
- **Goal:** Delete `src/FlowTime.Core/Models/ModelValidator.cs` outright and route every call site (`POST /v1/run`, Engine CLI, `TimeMachineValidator`, tests) through `ModelSchemaValidator`. Directly enforces the 2026-04-23 Truth Discipline guard *"'API stability' does not mean 'keep old functions around.'"*
- **Milestone status post E-24 close:** m-E23-01 largely absorbed by E-24 (`model.schema.yaml` rewritten by m-E24-03; canary committed and now hard-asserts under m-E24-05); needs a status-reconciliation pass on the stashed branch · m-E23-02 ready (E-24 prerequisite cleared) · m-E23-03 ready (waits on m-E23-02)
- **Dependencies:** E-24 Schema Alignment (cleared 2026-04-25)
- **Unblocks (after E-23 closes):** m-E21-07 Validation Surface (Svelte)
- **Regression canary:** `tests/FlowTime.Integration.Tests/TemplateWarningSurveyTests.cs` — promoted to hard assertion at m-E24-05 (`val-err == 0` in-process tier-3 survey across all 12 templates; build-breaks on any future regression)
...
- **Status:** **Paused (2026-04-23)** — interrupted to run E-23 Model Validation Consolidation. m-E21-01/02/03/04/05/06 complete (m-E21-01/02 merged 2026-04-17; m-E21-03 merged 2026-04-17 with ultrareview follow-ups 2026-04-20; m-E21-04 completed 2026-04-22; m-E21-05 completed 2026-04-22; m-E21-06 Heatmap View completed 2026-04-24 on branch `milestone/m-E21-06-heatmap-view` but **not yet merged** to `epic/E-21-svelte-workbench-and-analysis` — merge happens when E-21 resumes). Scope split 2026-04-21 during m-E21-04: Optimize moved to m-E21-05; downstream renumbered heatmap → m-E21-06, validation → m-E21-07, polish → m-E21-08 (epic now has 8 milestones, was 7). **Reentry point when resumed:** m-E21-07 Validation Surface, which consumes the consolidated `ModelSchemaValidator` E-23 delivers. **Remaining after resume:** m-E21-07 Validation Surface, m-E21-08 Polish.
