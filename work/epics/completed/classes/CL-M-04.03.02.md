...
Each phase follows FlowTime guardrails (TDD order, tracker updates, full `dotnet build` + `dotnet test` before hand-off). Adoption guidance and analyzer output must be recorded in `work/epics/completed/classes/CL-M-04.03.02-log.md` when the milestone kicks off.
