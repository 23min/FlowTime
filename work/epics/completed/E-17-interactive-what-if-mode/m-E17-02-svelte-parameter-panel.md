# Milestone: Svelte Parameter Panel

**ID:** m-E17-02
**Epic:** E-17 Interactive What-If Mode
**Status:** complete
**Branch:** `milestone/m-E17-02-svelte-parameter-panel` (merged to main)
**Depends on:** m-E17-01 (WebSocket engine bridge + EngineSession client)
...
After m-E17-01, the `/engine-test` smoke test page proves the plumbing works: browser → .NET WebSocket proxy → Rust engine session, round-trip in milliseconds. But the smoke test only tweaks one hard-coded parameter and displays series as flat text. It's a plumbing verification, not a user experience.

m-E17-02 builds the real interactive surface:
...
The emphasis is **interactive feel**: the user should be able to grab a slider and see numbers move immediately. Chart/topology visualizations are m-E17-03 — this milestone shows series as small-multiple sparklines or numeric tables that update on parameter changes.
...
9. **AC-9: Series display.** For every non-internal series in the eval response, the UI shows:
   ...
   The sparkline's y-axis auto-scales to the series' min/max. This is intentionally lightweight — full charting is m-E17-03.
...
- Full interactive charts with hover, zoom, legends (m-E17-03)
- Topology graph visualization with heatmap (m-E17-03)
...
- `ui/src/lib/api/engine-session.ts` — EngineSession WebSocket client (m-E17-01)
