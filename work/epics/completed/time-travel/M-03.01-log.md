...
- **Milestone Document:** [`work/epics/completed/time-travel/M-03.01.md`](M-03.01.md)
- **Roadmap Context:** [`work/epics/completed/time-travel/time-travel-planning-roadmap.md`](../../architecture/time-travel/time-travel-planning-roadmap.md)
...
### 2025-10-17 — Observability Assertions & Docs Refresh
...
**Tests:**
- ✅ `dotnet test tests/FlowTime.Api.Tests --logger "console;verbosity=detailed"`
- ✅ `dotnet test tests/FlowTime.Core.Tests --logger "console;verbosity=minimal"`

**Next Steps:**
- [ ] Revisit Phase 1 Task 1.3 schema documentation deliverable.
- [x] Smoke `.http` samples against a running API instance prior to milestone closure.
- [x] Follow-up: add wildcard (`*`) expansion support to `RunArtifactWriter` outputs so `.http` samples emit CSVs without explicit series lists.
- [x] Follow-up: update `RunArtifactWriter` to persist the canonical `model/metadata/provenance` artifact layout so runs created via `/v1/run` can be replayed by `/state` without manual fixes.

---

### 2025-10-18 — Canonical Artifact Writer Upgrade

**Changes:**
- Extended `RunArtifactWriter` to emit canonical `model/model.yaml`, `metadata.json`, and `provenance.json`, while keeping legacy `spec.yaml` for compatibility.
- Added wildcard (`*`) and prefix (`foo/*`) expansion when resolving outputs so time-travel specs can reference all evaluated series without explicit enumeration.
- Updated provenance-focused tests to resolve the canonical artifact path and validated the new `docs/schemas/time-travel-state.schema.json`.

**Tests:**
- ✅ `dotnet test tests/FlowTime.Tests`
- ✅ `dotnet test tests/FlowTime.Api.Tests --logger "console;verbosity=minimal"`
...
### 2025-10-19 — Time-Travel Schema Validation

**Changes:**
- Added JSON Schema validation (`StateResponseSchemaTests`) ensuring `/state` and `/state_window` payloads conform to `docs/schemas/time-travel-state.schema.json`.
- Documented the schema in `docs/schemas/README.md` and closed Phase 1 Task 1.3.
...
**File(s):** `docs/schemas/time-travel-state.schema.json`, `work/epics/completed/time-travel/M-03.01.md`

**Checklist (TDD Order):**
- [x] Draft schema validation tests ensuring required fields present (RED)
- [x] Publish JSON Schema or Markdown contract and link from docs (GREEN)
- [x] Regenerate or approve documentation snapshots (GREEN)
...
**File(s):** `src/FlowTime.API/FlowTime.API.http`, `work/epics/completed/time-travel/time-travel-planning-roadmap.md`
