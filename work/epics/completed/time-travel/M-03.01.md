...
**Status:** ✅ Complete (released 2025-10-19)  
...
### Strategic Context
- **Motivation:** The roadmap (`work/epics/completed/time-travel/time-travel-planning-roadmap.md`) calls for Engine-owned `/state` and `/state_window` endpoints before UI work can progress.
...
### Completion Summary
- Engine endpoints, contracts, telemetry-aware validation, and canonical artifact persistence shipped on 2025-10-19 (see `docs/releases/M-03.01.md`).
- Unit, integration, and golden tests (`dotnet test FlowTime.sln`) cover the acceptance criteria; `.http` walkthroughs are updated.
- Live ADX ingestion is deferred to a future (TBD) milestone and is not required for M-03.01.
...
**Acceptance Criteria:**
- [x] Responds `200 OK` with the JSON contract below when `runId` and `binIndex` are valid.
- [x] Embeds `metadata.schema`, `metadata.mode`, and `metadata.provenanceHash` sourced from the run manifest.
- [x] Includes `bin.startUtc`, `bin.endUtc`, and `bin.index` computed from the window grid.
- [x] Returns `404` for unknown `runId`, `400` for out-of-range `binIndex`, and `422` when required topology/window metadata is missing.
...
**Acceptance Criteria:**
- [x] Supports `startBin` and `endBin` inclusive, enforcing `0 <= startBin <= endBin < window.bins`.
- [x] Returns `series.timestampsUtc[]` with length `endBin - startBin + 1`.
- [x] Emits per-node arrays for `arrivals`, `served`, `capacity`, `queue`, and derived metrics aligned with timestamps.
- [x] Returns `400` for inverted ranges or slices exceeding the configured maximum window length (default 288 bins).
...
**Acceptance Criteria:**
- [x] Responses include `metadata.mode` (`simulation` or `telemetry`) and optional `metadata.telemetrySourcesResolved` boolean.
- [x] Simulation runs return `422` on missing metrics that violate schema, while telemetry runs aggregate missing/zero data into `warnings[]`.
- [x] Provenance hash from storage must match the requested `runId`; mismatches trigger `409` with expected hash information.
- [x] Validation failures emit structured error codes (`mode_validation_failed`, `provenance_mismatch`, etc.) documented for client consumption.
...
### Schema References
- `work/epics/completed/time-travel/time-travel-architecture-ch4-data-flows.md` — expected data contracts for UI.
...
**Success Criteria:**
- [x] Contracts serialise/deserialise round-trip in unit tests.
- [x] Manifest reader resolves mode/hash for simulation and telemetry fixtures.
...
**Success Criteria:**
- [x] `/state` returns expected payload for fixture run (`dotnet test` integration suite).
- [x] `/state_window` returns aligned series for slice and rejects invalid ranges.
- [x] Provenance mismatches surface as `409` with detailed error body.
...
**Success Criteria:**
- [x] Golden tests detect schema changes.
- [x] `.http` samples render full metadata envelope.
- [x] Logs include run id, mode, slice size, and validation outcome.
...
- [x] `/state` and `/state_window` endpoints implemented with documented payloads and guardrails.
- [x] Mode-aware validation differentiates simulation vs telemetry behaviours.
- [x] Provenance hashes validated and surfaced in metadata + error responses.
- [x] Unit, integration, and golden tests passing (`dotnet test FlowTime.sln`).
- [x] Documentation (`FlowTime.API.http`, roadmap references) reflects new endpoints and payloads.
- [x] Logging/observability requirements verified.
...
- `work/epics/completed/time-travel/time-travel-planning-roadmap.md` — Update milestone status references.
- `src/FlowTime.API/FlowTime.API.http` & `src/FlowTime.Sim.Service/FlowTime.Sim.Service.http` — Add request examples.

### Files to Modify (Minor)
- `work/epics/completed/time-travel/time-travel-planning-decisions.md` — Cross-reference validation decisions if phrasing changes.
...
- `work/epics/completed/time-travel/time-travel-architecture-ch4-data-flows.md`
- `work/epics/completed/time-travel/M-03.00.md`, `work/epics/completed/time-travel/SIM-M-03.00.md`
