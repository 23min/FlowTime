...
- UI consumers can rely on current `/state` contracts; `/v1/runs` now covers create/list/detail (with dry-run support), though topology layout metadata is still missing.
...
- **Run orchestration service**: Shared `RunOrchestrationService` powers `/v1/runs` create/list/detail endpoints (`src/FlowTime.API/Endpoints/RunOrchestrationEndpoints.cs:1`) and the CLI wrapper `flowtime telemetry run` (`src/FlowTime.Cli/Commands/TelemetryRunCommand.cs:1`), exposing dry-run planning, deterministic IDs, and shared manifest warnings across API/CLI flows.
- **Run discovery UX**: `/v1/runs` supports mode/template/warning filters plus page/pageSize pagination while `/v1/runs/{runId}` now surfaces `canReplay` so operators can gate `/state` calls.
- **Orchestration telemetry**: `RunOrchestrationService` emits structured `run_created` / `run_failed` logs with template, run id, and mode tags to feed observability pipelines.

## Inconsistencies and Documentation Gaps
- **Milestone status drift**: `work/epics/completed/time-travel/M-03.01.md`, `work/epics/completed/time-travel/M-03.02.md`, and `work/epics/completed/time-travel/SIM-M-03.00.md` still claim Planned/In Progress despite code completion; tracking logs disagree.
- **Sim implementation plan mismatch**: `work/epics/completed/time-travel/sim/sim-implementation-plan.md` notes WS1/WS2 as unscheduled even though work is merged.
...
1. **Document API/CLI workflow**: Socialise the refreshed `.http` samples and operator guide updates for `/v1/runs`, capture operator feedback, and fold it into milestone sign-off.
...
- Scripts such as `scripts/time-travel/run-sim-template.sh` still help assemble custom scenarios, but `/v1/runs` + `flowtime telemetry run` now provide the primary orchestration path.
