# M-03.00.01 — Shared Expression Validation Library
...
- FlowTime.Expressions validator is wired through ModelParser with regression tests covering shared error messaging.
- Manual `.http` workflows now include both canonical state queries and a validation-failure sample (SHIFT without initial condition) for manual verification.
- `dotnet test FlowTime.sln` is green after adding deterministic warm-up passes to the performance harness (`M2PerformanceTests`, `M15PerformanceTests`) to smooth JIT/GC variance.
...
- Shared validation result emits canonical error payload (`Validate_ReturnsError_WhenSelfShiftDetected`, `Validate_ReturnsSuccess_WhenNoSelfShiftDetected`).
- API integration propagates shared validation errors (see `GetState_WithSelfShiftMissingInitial_ReturnsConflict`).
- Integration smoke test: `flowtime run` on existing fixtures to ensure outputs match pre-refactor results.

### Manual Verification
1. Use the HTTP scratch file at `src/FlowTime.API/FlowTime.API.http` to POST the canonical time-travel model and capture the returned `runId`.
2. Call `/v1/runs/{runId}/state?binIndex=1` and `/v1/runs/{runId}/state_window?startBin=0&endBin=3` to confirm shared validation errors and derived metrics materialise in API responses.
3. Inspect the generated series metadata via `/v1/runs/{runId}/index` to ensure topology fixtures align with FlowTime.Expressions output.

---

## Success Criteria
- [x] `FlowTime.Expressions` project added with complete parser/validation implementation.
- [x] Shared test suite (`FlowTime.Expressions.Tests`) exercises parser and validation logic.
- [x] Engine builds/tests passing; no behavioural changes observed.
- [x] Documentation updated to direct Engine/Sim teams to the shared library.
- [x] Milestone tracking doc (`work/epics/completed/time-travel/M-03.00.01-log.md`) reflects completion.
...
## References
- `work/epics/completed/time-travel/time-travel-planning-roadmap.md` (Post-M-03.00 follow-up #1)
- `work/epics/completed/time-travel/sim/sim-implementation-plan.md` (WS2 dependency)
- `work/epics/completed/time-travel/sim/sim-architecture-overview.md` (shared validation requirement)
