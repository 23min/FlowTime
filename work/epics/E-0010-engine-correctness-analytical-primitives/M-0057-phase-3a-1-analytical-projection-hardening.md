...
acs:
  ...
  - id: AC-5
    ...
    status: met
  - id: AC-6
    title: 'AC-6: Remaining purification work is explicitly handed off to E-0016'
    status: met
  - id: AC-7
...
Establish the first correct boundary move for analytical state metrics: Core owns the current analytical capability resolution and derived metric computation used by `/state` and `/state_window`, while the remaining formula-first purification work is explicitly handed to E-0016.
...
### AC-6 — AC-6: Remaining purification work is explicitly handed off to E-0016

**AC-6: Remaining purification work is explicitly handed off to E-0016.** Typed semantic references, runtime analytical descriptors, class-truth separation, public analytical contract redesign, client heuristic deletion, and final semantic-parser removal are out of scope for `m-ec-p3a1` and owned by E-0016.
...
- `AnalyticalCapabilities` remains the bridge abstraction for the current state surfaces. E-0016 will replace string-derived inputs with compiled runtime facts.
...
- Semantic string parsing and logicalType reconstruction that still exist for runtime behavior are tolerated only as bridge behavior. E-0016 owns deleting them.
...
- E-0016 follows immediately after wrap and before Phase 3 continues with p3b/p3c/p3d
