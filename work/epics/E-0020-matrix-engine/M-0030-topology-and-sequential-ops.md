---
id: M-0030
title: Topology and Sequential Ops
status: done
parent: E-0020
acs:
  - id: AC-1
...
M-0029 delivered the compiler and evaluator for flat models (const + expr + PMF). The compiler produces a plan of element-wise ops, the evaluator executes against a flat matrix. 38 Rust tests passing.
...
**AC-4: SHIFT feedback.** Models with SHIFT-based cross-node feedback cycles evaluate correctly without special handling. The Shift op reads `state[input, t - lag]` which was written in a previous bin iteration since the evaluator processes ops in plan order and sequential ops process bins in order. Test: the backpressure model from E-0010 p3b (queue → pressure → SHIFT → effective_arrivals → queue) produces the same stabilization pattern.
...
- **Topology node ID → queue column:** The compiler maintains a mapping from topology node ID to the synthesized queue column index, used for overflow routing and later for derived metrics (M-0032).
...
- Routing (router flow materialization) — M-0031
- Constraints (proportional allocation) — M-0031
- Multi-class flows — M-0031
- Derived metrics (utilization, latency, etc.) — M-0032
- Invariant analysis — M-0032
- Artifact writing — M-0033
- File-based series references (`file:*.csv`) — future (telemetry mode)

## Dependencies

- M-0029 complete (compiler + evaluator + element-wise ops)
