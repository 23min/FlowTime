...
M-0031 delivered routing and constraint allocation. The engine now handles the full evaluation pipeline: const/expr/PMF nodes, topology synthesis (queues, retry echo, dispatch), WIP overflow, SHIFT feedback, routers, and constraints. 78 Rust tests passing.
...
- Artifact writing (CSVs, index.json, run.json) — M-0033
- CLI commands — M-0033
...
- Streak detection (backlog growth, overload, age risk) — future (could add in M-0032 if time permits)

## Dependencies

- M-0031 complete (routing, constraints, full evaluation pipeline)
