...
# E-26 — Local + CI Test Discipline and Static Analysis
...
- **No snapshot / golden-file testing.** M-068 (Golden-Output Canary, in flight under E-25) will hand-roll a fixture-comparison harness because Verify.Xunit / insta are not in the dep tree. Adopting Verify and insta would simplify M-068's harness, give every future milestone the same primitive, and standardize the regeneration workflow CLAUDE.md hints at but has no convention for.
...
### Coordination with E-25

E-25's M-068 (Golden-Output Canary) and this epic's Tier 2 (which introduces Verify.Xunit and insta) overlap in scope. The cheapest sequencing is to land E-26 Tier 2 **before** M-068 starts implementation, so M-068 builds on `Verify` instead of hand-rolling fixture comparison. If sequencing reverses (M-068 ships first), Tier 2 becomes a refactor of M-068's hand-rolled harness onto Verify. Both orderings work; the first is cheaper. E-25 is not formally blocked on this epic — Tier 1 in particular can land in parallel with M-067/M-069 with no interaction.
...
- **Promoting the InvariantAnalyzer warnings to CI gate.** That work is already filed as gap [G-035](../../gaps/G-035-promote-invariantanalyzer-warnings-to-ci-gate.md) and is logically separate (it gates on engine output, not on toolchain). G-035 may be addressed alongside or after this epic; it is not part of this epic's scope.
...
| Snapshot-test format — Verify default (`.received.txt` / `.verified.txt`), JSON, or YAML? | No | Decided inside the Tier 2 milestone alongside the M-068 fixture-format choice (per the *Coordination with E-25* note). Strawman: JSON-with-stable-key-order, matching what M-068 will need anyway. |
...
| Adopting Verify / insta now diverges from the M-068 fixture-format choice if M-068 ships first. | Low | This epic flags the coordination explicitly. If M-068 ships first with a hand-rolled comparator, the Tier 2 milestone refactors M-068's harness onto Verify in the same change that introduces Verify. The refactor is mechanical — the comparator semantics are identical. |
...
<!-- Sequencing rationale: Tier 1 lands first because it is broad-coverage cheap-cost and unblocks every later
     ...
     adopting Verify/insta in Tier 2 simplifies E-25's M-068 if Tier 2 lands first. Tier 3 lands third because
...
- [M-071 — **Tier 2: Property + snapshot + mutation + architecture testing**](M-071-tier-2-property-snapshot-mutation-architecture-testing.md) — FsCheck/CsCheck or proptest; Verify.Xunit and insta; Stryker.NET and cargo-mutants on a manual/nightly job; NetArchTest with the seeded rule list. CLAUDE.md updates: test-type selection rule, snapshot hygiene rule, strengthened TDD rule. Coordinate with E-25 M-068. · depends on: M-070
- [M-072 — **Tier 3: Fuzzing + coverage floor + supply-chain scan**](M-072-tier-3-fuzzing-coverage-floor-supply-chain-scan.md) — cargo-fuzz and SharpFuzz targets; coverlet.collector with floor; `dotnet list package --vulnerable` and `cargo deny check advisories` in CI; allow-list mechanism with first D-NNN entry seeded if needed. · depends on: M-071

The CLAUDE.md updates are not a dedicated milestone — they ride along with each tier's milestone (each milestone updates the rules its primitives introduce). If the user wants a dedicated milestone for the CLAUDE.md sweep instead, the sequencing splits cleanly: M-070 (Tier 1), M-071 (Tier 2), M-072 (Tier 3), M-073 (CLAUDE.md sweep + final polish).

## References

- Gap [G-035](../../gaps/G-035-promote-invariantanalyzer-warnings-to-ci-gate.md) — adjacent CI gate work; logically separate but motivates the same "make process discipline mechanical" framing this epic adopts.
- Epic [E-25](../E-25-engine-truth-gate/epic.md) — Engine Truth Gate; M-068's golden canary work overlaps with this epic's Tier 2 (Verify / insta). See *Context → Coordination with E-25*.
- `Directory.Build.props` — current state; modified by M-070.
- `.editorconfig` — current naming + Roslynator codes; expanded by M-070.
- `engine/Cargo.toml` — current state (no `[workspace.lints]`); expanded by M-070.
