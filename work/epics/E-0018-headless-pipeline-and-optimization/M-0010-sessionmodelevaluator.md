...
`SessionModelEvaluator` uses the M-0002 session protocol (MessagePack over stdin/stdout):
...
- `RustEngineRunner` remains `Singleton` (still used by E-0020 bridge/parity tests and by
  `RustModelEvaluator`).
...
- M-0002 (engine session protocol) — delivered
- M-0005 (ITelemetrySource) — independent
- M-0006 (`IModelEvaluator` seam, `ConstNodePatcher`) — delivered
- M-0007 (`ConstNodeReader`) — delivered
