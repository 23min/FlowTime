...
After M-0047 `ModelValidator` has zero production callers and its dedicated test files have been rewritten to target `ModelSchemaValidator`. The type is dead weight — kept on disk only as a rollback seam during M-0047. This milestone removes that seam.
...
**Both canaries still green.** `TemplateWarningSurveyTests.Survey_Templates_For_Warnings` reports `val-err == 0` across all twelve templates at `ValidationTier.Analyse` (E-0024 M-0053's hard-asserting survey canary), and `RuleCoverageRegressionTests` (M-0046's negative-case catalogue) reports green — every rule the audit claimed `ModelSchemaValidator` covers, it still catches after the delete. Both canaries are the final confirmation that nothing depends on `ModelValidator` post-deletion.
...
- Rollback must remain a single `git revert` for at least the M-0048 merge commit. Keep the delete in one commit; do not mix in any other refactor.
...
- Any change to `ModelSchemaValidator.cs` — if the delete reveals `ModelSchemaValidator` is missing a rule the audit should have caught, treat that as an M-0046 scope escape (the negative-case canary should have caught it; if it didn't, the audit was incomplete).
