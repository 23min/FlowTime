...
Land the schema/compile/analyse enforcement points named by the M-0066 ADR so the flow-authority policy is enforced at every gate, not just documented. The deliverable is engine + validator + analyser code that rejects models violating the policy, plus tests proving each gate fires on deliberately-broken model fixtures. **No template edits, no `ExpectedRunWarnings` baseline reset**: this milestone makes the gates real; the next milestone (M-0067 Engine + Template Alignment) edits templates so they pass under the new gates.

## Context

M-0066 ratified an ADR naming the flow-authority policy: routing authority lives at producer-fan-out points, edge weights are normative for static-share fan-outs (class 3), router nodes for dynamic routing (class 1), class-2 capacity-aware allocation is not surfaced today and is filed as a deferred gap, consumer-side expr arithmetic must not encode peer-relative splits. The ADR explicitly named three enforcement layers:
...
This milestone implements those three layers. It does NOT edit the existing 9 affected shipped templates — that's M-0067's job. The existing shipped template suite is expected to **fail** the new gates after this milestone lands; that failure is the proof that the gates work, and M-0067's job is to fix the templates so they pass.

The `ExpectedRunWarnings` baseline canary stays exactly as it is. The new gates produce *errors* and *warnings* that are independent of the existing run-warn baseline. M-0067 will reset the baselines once the templates are aligned.
...
- Templates that the ADR names as policy-violating (the 9 affected from G-0032) **fail one or more gates** (likely AC-1 schema-level or AC-4 analyser warning). This failure is the proof that the gates work; M-0067 fixes the templates.

The exact failure mode for each of the 9 templates is documented in this milestone's spec body before close. M-0067 plans against this documented failure mode.

The Phase 2 `ExpectedRunWarnings` baseline canary is unchanged; the existing run-warn baselines stay exactly as they are. The new gate failures are *separate* signals that M-0067 will collapse to clean-by-construction.

### AC-9 — Documentation updated to reference the gates

The taxonomy doc from M-0066 (`docs/architecture/flow-authority-policy.md` or its M-0066-chosen name) is updated with a "Enforcement" section that names the four gates landed by this milestone, what each rejects/warns on, and how an author can debug a violation. The testing doc that lands in M-0068 (`docs/testing/golden-output-canary.md`) is not touched here — that's M-0068's deliverable.
...
- **No template edits.** The 9 affected shipped templates from G-0032 are not touched by this milestone. Their edits land in M-0067. This milestone's job is to make the gates real and prove they fire on the broken patterns.
- **No `ExpectedRunWarnings` changes.** The baseline canary stays exactly as it is. Resetting baselines is M-0067's job once the templates are aligned. This milestone's new gates are independent of the existing run-warn baseline.
...
- **Where the schema rule lives.** Three options: (a) JSON Schema additions to `docs/schemas/model.schema.yaml` (limited expressive power); (b) a custom validator rule in `ModelSchemaValidator` that runs after JSON-Schema parsing (full C# expressive power); (c) both (schema-level for the simple case, validator-level for the AST-pattern case). Strawman: option (b) for the AST-pattern detection, with a schema-level pattern hint if a useful subset can be expressed declaratively. The ADR's enforcement section in M-0066 may prefer one over the others; pick whichever matches the ADR.
- **Where the compile detector lives.** Two options: (a) a new pass in `ModelCompiler`; (b) a new validation in `TimeMachineValidator`'s tier-2 compile path. Strawman: option (b), because `TimeMachineValidator` is already the canonical home for tier-N validation per E-0023.
...
- Template edits (M-0067).
- `ExpectedRunWarnings` baseline reset (M-0067).
- The `val-warn` delta gate addition to `Survey_Templates_For_Warnings` (M-0067 — the gate addition is part of M-0067's bridge canary work).
- Class-2 capacity-aware allocator implementation (deferred per M-0066's gap; future engine work).
- Rust-engine alignment with the policy (G-0016 territory; future work).
- Golden-output canary infrastructure (M-0068).
...
- M-0066 — ratified ADR naming the policy and the enforcement points. M-0069 cannot start until the ADR is `accepted`.
...
- Compiler source: `src/FlowTime.TimeMachine/...` (per E-0024's unified shape)
