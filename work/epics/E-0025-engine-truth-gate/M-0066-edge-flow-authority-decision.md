...
acs:
    - id: AC-1
      ...
      status: met
    - id: AC-2
      title: Footprint analysis preserved for the three G-0032 options
      status: met
    - id: AC-3
    ...
    - id: AC-9
      ...
      status: met
    - id: AC-10
      title: Epic spec and G-0032 reference the ratified ADR
      status: met
    - id: AC-11
...
This milestone began life as "edge-flow authority decision" — pick one of three options listed in G-0032 (edge weights win / expr authority wins / both-must-agree) and ratify a `D-NNN`. During M-0066's working session the framing widened: the question is not just "which surface is normative for class-3 fan-outs" but **"what is FlowTime's flow-authority policy across the three classes of physical systems we exist to model, and how do we enforce it at every layer of the engine"**.
...
- **Original option 1 — edge weights win — is the right answer for class-3 fan-outs**, which is the case all 9 affected templates from G-0032 actually instantiate. Edge weights ARE the routing rule for class 3.
...
### AC-2 — Footprint analysis preserved for the three G-0032 options

The footprint analysis already produced during this milestone (preserved inline in this spec under "Footprint analysis (M-0066's working artefact)") documents engine-LOC magnitude, per-template diff estimates, and baseline-zero achievability for each of the original three G-0032 options. The preserved analysis lets a future reader see why the chosen authority won on flow-purity grounds *and* on engineering-cost grounds.

### AC-3 — Repo-wide doc sweep classifies every routing-relevant document

A doc-sweep classification preserved as a section of this spec ("Doc-sweep classification (M-0066's AC-3 artefact)") classifies every routing-relevant document in the repo against the policy. Search axes:
...
- **Axis 3 — ADRs and decision records.** `docs/adr/` is empty today. Decision records: D-0046 (`GET /v1/runs/{runId}/model`), D-0047 (`trace` field on goal-seek/optimize), D-0051 (E-0024 schema-alignment closure), D-0053 (testing rigor). Read each for routing-semantic implications.
- **Axis 4 — gaps that may need revision.** G-0032 (already addressed by this milestone), G-0016 (Rust Engine Parity), G-0018 (`IModelEvaluator` Series-Key Shape Divergence), G-0019 (Sim-generated model shape vs. Rust engine compiler expectations), G-0003 (Dependency Constraint Enforcement — touches capacity → class 2), G-0011, G-0013 (fit/calibration → routing model assumptions).

For each hit, classify into one of: **aligned**, **silent**, **conflicting**, **ambiguous**. Output is one section per hit in the inlined doc-sweep classification (this spec's "Doc-sweep classification (M-0066's AC-3 artefact)" section), with a one-sentence note on what (if anything) needs to change.
...
The point: no doc classified as conflicting may survive M-0066 close in its current state without an explicit pointer to the ADR.
...
## Footprint analysis (M-0066's working artefact)
...
For each of the three G-0032 options (edge weights win / expr authority wins / both-must-agree), one representative template is walked under that option's resolution; the diff is counted; the per-template cost is extrapolated across the 9 affected shipped templates listed in G-0032. Engine-LOC magnitude is bucketed: `small` (< 50 LOC, single file), `medium` (50–300 LOC, 1–3 files), `large` (300+ LOC or multi-subsystem). Anything `large` triggers the epic risk-table mitigation (split into a multi-milestone implementation phase rather than extending E-0025 indefinitely).

### Affected templates

Sourced from G-0032's "Affected templates" table and confirmed against `ExpectedRunWarnings` in `tests/FlowTime.Integration.Tests/TemplateWarningSurveyTests.cs:79`. Nine templates are affected:
...
| 2 — Expr authority wins | `medium` (150-300 LOC; `EdgeFlowMaterializer.cs` + `InvariantAnalyzer.cs` + possibly `RouterSpecificationBuilder.cs`) | 0 | Yes for all 9 by definition | No template breakage, but edge weights become advisory for a class of edges — semantic shift with E-0015 / E-0022 downstream consequences |
...
This section classifies every routing-relevant document in the repository against the flow-authority policy. The classification feeds AC-4 (which decides whether each conflicting doc is revised in M-0066 or marked needs-revision and deferred). Search axes follow the M-0066 spec under AC-3:
...
|---|---|---|---|
| `docs/architecture/flow-authority-policy.md` | aligned | The policy doc itself (M-0066 AC-1 deliverable). | No change. Reference target for ADR-0001. |
...
| `docs/reference/flow-theory-coverage.md` | conflicting | Section 8 "Routing & Flow Splitting" (lines 111–118): row `Dynamic routing (route by downstream queue length) \| **Not planned**`. The flow-authority policy explicitly classifies this as class-2 (capacity-aware allocation) and frames it as a *deferred future capability*, not "not planned" (the deferred-follow-up gap is filed as G-0038 under AC-7). | **AC-4 revise (small).** Change the "Not planned" row to "**Deferred** — class-2 capacity-aware allocation, see ADR-NNNN and G-0038 (class-2 capacity-aware allocator gap, M-0066/AC-7)." Single-cell edit. |
...
`docs/adr/` does not exist today; ADR-0001 (the M-0066 deliverable under AC-5) seeds it. No prior ADRs to classify.

For decision records under `work/decisions/`, the M-0066 spec names D-0046, D-0047, D-0051, D-0053:
...
Two conflicting docs require small in-place revisions in M-0066 AC-4:
...
2. **`docs/reference/flow-theory-coverage.md` lines 111–118** — change the "Not planned" verdict for dynamic routing to "Deferred — class-2 capacity-aware allocation" with a forward reference to ADR-NNNN and G-0038. Diff size: one cell.

Both are within the AC-4 "small revision" threshold (paragraph or section that can be replaced without changing the doc's overall structure). No `[needs revision per ADR-NNNN]` markers are required — both can be revised in M-0066.

No deferred follow-up gaps (AC-8) surfaced from this sweep beyond G-0038 (the class-2 capacity-aware allocator gap filed under AC-7). The possible class-3 enhancement to `template-authoring.md` (treat edge-as-channel as a first-class authoring pattern alongside the router) is enhancement work — not a contradiction with the policy.

**Class-3 authoring guidance: folded into M-0067 scope; no new gap.** The engine + template alignment milestone (M-0067) already revisits authoring guidance as part of its surface; the class-3 first-class-authoring enhancement is in scope for that milestone and does not need an independent AC-8 gap.
...
- **Footprint analysis is preserved verbatim.** The footprint analysis section in this spec is M-0066's working artefact and is preserved per AC-2. Editing it is out of scope unless a factual error is discovered; the framing-widening (flow-purity reframe) does not invalidate the cost evidence.
...
- **What "revise in this milestone" means for AC-4.** A "small" revision is a paragraph or section that can be replaced or rewritten without changing the doc's overall structure. Anything larger gets the `[needs revision per ADR-NNNN]` mark and a deferred follow-up gap. Goal: cap M-0066's doc-edit scope so the milestone closes promptly; the heavy doc-rewrite work belongs to a future milestone or wf-patch.
...
- **G-0037 already names some of the doc-sweep hits.** During M-0066's working session a sibling gap G-0037 was filed: "Pre-aiwf v1 framework docs survived migration and contradict the v3 model." It enumerates several `docs/development/*.md` files with v1 conventions that contradict the v3 model. The doc sweep (AC-3) should pick up where G-0037 leaves off — those v1 docs may also contain routing-semantic claims that need the same classification pass.
...
- Class-2 capacity-aware allocator implementation (filed as deferred gap per AC-7; future engine work, not E-0025 scope).
- Any change to the `Survey_Templates_For_Warnings` test (Engine + Template Alignment owns the val-warn delta gate addition).
- Heavyweight rewrites of pre-aiwf v1 documentation (G-0037's territory; M-0066 marks conflicting docs but does not rewrite them past the small-revision threshold defined in AC-4).
...
- **2026-05-05** — per-AC closeout (commit SHAs at the time of the wrap):
    ...
    - **AC-2** — footprint analysis preserved verbatim in this spec under "Footprint analysis (M-0066's working artefact)"; promoted met (commit `377b945`).
    - **AC-3** — repo-wide doc sweep inlined into this spec under "Doc-sweep classification (M-0066's AC-3 artefact)" (commit `a27bce2`); promoted met (commit `f8167bc`). 42 docs in scope; 10 aligned, 30 silent, 2 conflicting, 0 ambiguous.
    - **AC-4** — both conflicting docs revised in-place: `docs/flowtime-v2.md` line 246 (universal fan-out row split into class-3 + class-1 rows) and `docs/reference/flow-theory-coverage.md` lines 111–118 (Not planned → Deferred with forward refs to ADR-0001 and G-0038) (commit `d133c5a`); promoted met (commit `ab5f529`).
    ...
    - **AC-7** — G-0038 filed via `aiwf add gap` capturing class-2 capacity-aware allocator as deferred future capability (commit `df84886`); milestone reference resolved from G-NNN to G-0038 (commit `9bce97f`); promoted met (commit `24856cd`).
    - **AC-8** — sweep surfaced no further deferred follow-ups beyond G-0038; class-3 first-class authoring guidance recorded as folded into M-0067 scope (no new gap); promoted met (commit `6521313`).
    ...
    - **AC-10** — E-0025 epic spec open-questions table updated and G-0032 cross-referenced ADR-0001 (commit `245c7bc`); G-0032 promoted `open → addressed` (commit `b5da3fd`); promoted met (commit `f4eeacd`).
...
- **G-0038 — Class-2 capacity-aware allocator.** Filed under AC-7 as the deferred-follow-up gap for class-2 (consumer-side pull / capacity-aware allocation). Future engine capability, not current correctness. The gap names the modeling cases that require it (warehouse pickers, schedulers, admission control), why expr arithmetic cannot fake it correctly, and the proposed surface (a capacity-aware router actor distinct from the static-weight router). ADR-0001 references G-0038.
- **Class-3 first-class authoring guidance — folded into M-0067 scope (recorded for completeness; not a deferral).** The doc sweep noted that `docs/templates/template-authoring.md` does not yet describe class-3 (edge-as-channel) as a first-class authoring pattern alongside the router. Rather than file an independent AC-8 gap, this enhancement is in scope for the engine + template alignment milestone (M-0067), which already revisits authoring guidance as part of its surface.
