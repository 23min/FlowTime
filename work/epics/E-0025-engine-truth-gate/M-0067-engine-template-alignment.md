...
acs:
  - id: AC-1
    title: Engine reflects the M-0066 ADR chosen authority
    status: open
    tdd_phase: red
  - id: AC-2
...
Implement the flow-authority policy chosen in M-0066's ADR inside the engine, edit every affected shipped template so the conservation invariant produces no warnings under default parameters, reset the corresponding `ExpectedRunWarnings` entries to zero, and extend `Survey_Templates_For_Warnings` to gate on `val-warn` delta in addition to its existing `run-warn` gate. After this milestone, every shipped template runs warning-clean **and passes the M-0069 enforcement gates without modification**; both survey gates are active simultaneously, locking the engine into the new authority before M-0068 pins golden fixtures on top.

## Context

M-0066 ratifies an ADR (referenced below as `ADR-NNNN (the M-0066 outcome)` until the actual id is allocated and filled into this spec at start time) naming the **flow-authority policy** across three classes of physical systems FlowTime models. The ADR commits to: routing authority lives at producer-fan-out points, edge weights are normative for static-share fan-outs (class 3), router nodes for dynamic routing (class 1), class-2 capacity-aware allocation is filed as a deferred gap, consumer-side expr arithmetic must not encode peer-relative splits.

M-0069 lands the enforcement layers — the schema rule that rejects consumer-side peer-split patterns, the compile-time fan-out routing-authority detector, the two new analyser warnings (`routing_authority_ambiguous`, `consumer_side_peer_split_detected`). After M-0069 the engine **rejects** policy-violating models at `POST /v1/run`. After this milestone (M-0067), the existing 9 affected shipped templates from G-0032 are edited so they conform to the policy and pass the M-0069 gates. The transition is: M-0069 makes the gates real (templates fail), M-0067 fixes the templates (templates pass).

The engine implementation footprint of M-0067 is the AC-5 estimate from M-0066's footprint analysis; if the implementation overruns that estimate by more than a milestone's worth of work, the epic risk mitigation triggers a re-scope conversation rather than allowing this milestone to balloon.
...
`work/gaps/G-0032-…md` status moves from `addressed` (the M-0066 partial-close state) to `done` with a reference to this milestone. The epic spec is updated so its "Supersedes / closes" section reflects the closure.
...
- `src/FlowTime.Core/Analysis/InvariantAnalyzer.cs` (likely — under option 2; possibly under option 3) and/or other engine sites the M-0066 footprint analysis identifies
- `templates/*.yaml` (under option 1 or 3 — the affected subset from G-0032's table)
...
- `work/gaps/G-0032-…md` (status to `done`, reference this milestone)
...
- Golden-output canary infrastructure or fixtures (M-0068).
...
- Migration tooling for user-authored templates outside `templates/` (M-0066 AC-6 may file this as a deferred gap if applicable; the tool is not built here).
- UI changes (Svelte or Blazor) — none required by this epic.
- Promotion of the `D-NNN` to ADR — that decision lives in M-0066's closeout if it happens at all.

## Dependencies

- **M-0069 enforcement gates landed.** Hard prerequisite — this milestone edits templates so they pass the M-0069 gates; without the gates real, the templates have nothing concrete to pass.
- **M-0066 ratified ADR** with status `accepted`. Hard prerequisite — this milestone cannot start without the policy in place. M-0069 also depends on M-0066, so this is transitive.
- E-0025 epic spec ratified.
- Phase 2 baseline canary committed (2026-05-01) — provides the `run-warn` gate this milestone extends.
- E-0024 Schema Alignment closed (2026-04-25) — provides per-edge `flowVolume` series emission that the conservation invariant now reads.

## References

- Epic spec: `work/epics/E-0025-engine-truth-gate/epic.md`
- ADR: `ADR-NNNN (the M-0066 outcome)` — replace with actual id at milestone start
- Enforcement milestone: `work/epics/E-0025-engine-truth-gate/M-0069-schema-compile-analyse-enforcement.md` — gates that this milestone's template edits pass under
- Gap: `work/gaps/G-0032-transportation-basic-regressed-edge-flow-mismatch-incoming-3-after-e-24-unification.md` — affected-templates table; canonical source for AC-2 and AC-3 enumerations
...
- Pre-E-0024 reference run (clean baseline): `data/runs/run_20260424T150244Z_b2f4c995/run.json`
- Post-E-0024 reproduction case: `data/runs/run_20260428T165413Z_6ed5974e/run.json`
