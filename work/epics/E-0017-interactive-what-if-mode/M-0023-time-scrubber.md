...
After M-0022, both nodes and edges in the topology graph are colored by the **mean** of their series across all bins. The mean is a useful summary, but it collapses time — users cannot see:
...
The M-0020 spec explicitly noted this as a future enhancement:
...
**AC-5: `buildEdgeMetricMap` accepts optional bin.** The same optional `bin` parameter is added to `buildEdgeMetricMap` (from M-0022), with identical semantics.
