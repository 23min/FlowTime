...
acs:
  - id: AC-1
    title: 'AC-1: Warnings in protocol — compile response'
    status: met
  - id: AC-2
    title: 'AC-2: Warnings in protocol — eval response'
    status: met
  - id: AC-3
    title: 'AC-3: Example model that triggers a warning'
    status: met
  - id: AC-4
    title: 'AC-4: Warnings banner'
    status: met
  - id: AC-5
    title: 'AC-5: Warning details panel'
    status: met
  - id: AC-6
    title: 'AC-6: Node badge on topology graph'
    status: met
  - id: AC-7
    title: 'AC-7: Warnings update reactively on eval'
    status: met
  - id: AC-8
    title: 'AC-8: Pure unit tests'
    status: met
  - id: AC-9
    title: 'AC-9: Playwright E2E'
    status: met
  - id: AC-10
    title: 'AC-10: Compile error surface unchanged'
    status: met
---
...
### AC-1 — AC-1: Warnings in protocol — compile response

**AC-1: Warnings in protocol — compile response.** `CompileResult` (Rust `protocol.rs`, TS `engine-session.ts`) includes a `warnings: WarningInfo[]` field. The session handler populates it from the initial eval's `result.warnings`. Each entry carries `node_id`, `code`, `message`, `bins`, `severity`.
### AC-2 — AC-2: Warnings in protocol — eval response

**AC-2: Warnings in protocol — eval response.** `EvalResultMsg` includes the same `warnings: WarningInfo[]` field, populated from the eval's `result.warnings`. The field is **always present** — empty array means "no warnings," not "unknown".
### AC-3 — AC-3: Example model that triggers a warning
...
### AC-5 — AC-5: Warning details panel
...
### AC-6 — AC-6: Node badge on topology graph
...
### AC-7 — AC-7: Warnings update reactively on eval

**AC-7: Warnings update reactively on eval.** When a parameter change clears a warning, the banner, panel, and node badge all disappear. When a change introduces a warning, they all appear. No page reload.
### AC-8 — AC-8: Pure unit tests
