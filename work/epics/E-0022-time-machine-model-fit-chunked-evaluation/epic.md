...
E-0018 delivered 11 milestones covering parameterized evaluation, engine session protocol, tiered validation, parameter sweep, sensitivity, goal seek, N-parameter optimization, `SessionModelEvaluator`, and the .NET Time Machine CLI. All are on `main`. The analysis layer works end-to-end for synthetic runs.

Two scope items remained in E-0018 and are explicitly carried into E-0022:

- **Model Fit** (`FitSpec`/`FitRunner`/`POST /v1/fit`) — composes `ITelemetrySource` + `Optimizer` with residual as the objective. Infrastructure exists; the composition is the new work. **Blocked on** E-0015 Telemetry Ingestion (first repeatable dataset path) and the Telemetry Loop & Parity epic (validated drift bounds — fitting against real telemetry without measured drift would produce falsely precise results).

- **Chunked Evaluation** (Mode 6) — bin-chunk evaluation for feedback simulation with external controllers. The Rust engine session (M-0002) is the seam, but the chunk-step protocol on top of it is not designed.
...
Closes out E-0018 placeholder `m-E18-XX Model Fit` and `m-E18-05 Chunked Evaluation`. E-0018 itself remains the archived parent epic (11 delivered milestones recorded there as "complete"); E-0022 is the forward-looking epic that completes its scope.
...
- Test coverage matching E-0018 standard: 100% branch coverage on pure runners; integration tests for Rust chunk-step protocol; API contract tests for `/v1/fit` and `/v1/chunked-eval`
...
- **`FlowTime.Telemetry.*` direct-source adapters** (Prometheus, OpenTelemetry, BPI event logs) — E-0015 Gold Builder covers the general batch path; adapters are narrower shortcuts to build only when a concrete client asks
- **Tiered validation parity across Sim UI / Blazor UI / Svelte UI / MCP / external agents** — validation surface work; closer to E-0021 M-0043 (Validation Surface) and future Blazor maintenance
- **Canonical bundle round-trip parity AC** (E-0018 unchecked AC: capture baseline → replay bundle = same outputs modulo drift) — owned by Telemetry Loop & Parity, not E-0022
- **Fit-result UI** — lands in E-0021 (or a later Svelte milestone) once Fit's API is stable; E-0022 delivers only the contract
...
- **Time grid alignment across heterogeneous sources** — assumes E-0015 Gold Builder presents series on a common grid; grid alignment belongs to E-0015
- **Streaming / push UI for chunked evaluation** — chunked delivers a pull contract (caller drives chunk_step); WebSocket push for live simulation is a separate E-0017-style track

## Constraints

- Builds on the existing Rust engine session protocol (M-0002). No changes to the MessagePack framing; `chunk_step` is a new command alongside `compile`, `eval`, `patch`, `get_params`, `get_series`, `validate_schema`
...
- Byte-for-byte API ↔ CLI parity per the E-0018 CLI convention: `flowtime fit < request.json` produces the same JSON as `POST /v1/fit`
- .NET 9 / C# 13; invariant culture; camelCase JSON payloads (project-rule: `project.md:43`)
- No reintroduction of the deprecated `FlowTime.Generator` project (D-0032 Path B remains authoritative)
...
- [ ] All `/v1/*` API endpoints added by E-0018 are rewritten to call through `FlowTime.Pipeline` rather than `FlowTime.TimeMachine.*` internals directly; the SDK is dogfooded from day one
...
- [ ] On epic completion, E-0018's `m-E18-05 Chunked Evaluation` and `m-E18-XX Model Fit` placeholder rows are struck through; E-0018 epic status flips to `complete` and the epic is archived under `work/epics/completed/`

## Risks & Open Questions

| Risk / Question | Impact | Mitigation |
|-----------------|--------|------------|
| Telemetry Loop & Parity epic has not been scheduled; without it Fit cannot be meaningfully validated | **Hard block** | Telemetry Loop & Parity must be scheduled and completed before Fit AC can be closed. E-0022 does not start Fit until TLP ships |
| E-0015 first-dataset-path timeline is uncertain | High | E-0015 is on the critical path per Option A. E-0022 planning proceeds; E-0022 implementation is gated on E-0015 delivering at least one end-to-end canonical bundle dataset |
...
Sequencing: Fit first (unblocks the largest downstream set — E-0015 dataset path leads directly into fit validation). Chunked second (independent Rust protocol work; runs after Fit only to avoid Rust-engine contention on a single milestone branch). Pipeline SDK third so it crystallizes against the final surface including Fit and Chunked.

| ID | Title | Status | Depends on |
|----|-------|--------|-----------|
| m-E22-01-model-fit | Model Fit | not started | E-0015 first dataset path complete; Telemetry Loop & Parity harness validated |
| m-E22-02-chunked-evaluation | Chunked Evaluation | not started | m-E22-01 (sequencing only; no hard code dependency); M-0002 session protocol (already delivered) |
