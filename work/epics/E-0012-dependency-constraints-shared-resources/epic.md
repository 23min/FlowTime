---
id: E-0012
title: Dependency Constraints & Shared Resources
status: done
---

**Completed:** 2026-05-01

> Closure note (2026-05-01): M-0063 + M-0064 delivered the Option A (dependency-as-node) and Option B (dependency-as-constraint) representations. M-0065 (MCP Pattern Enforcement) was dropped from this epic and absorbed into the new draft `ai-assisted-authoring` unplanned epic at `docs/explorations/ai-assisted-authoring/spec.md` along with the original M-0065 Plans 3–5 (resource pooling, compiler expansion, feedback loops). E-0012 ships with both representations functional; AI-driven authoring hardening is the successor scope and lives outside the FlowTime UI surface (MCP-server-side, consumed by external authoring callers).
...
### Option A — Dependency as a Node (M-0063)
...
- **M-0063 MVP contract:** dependency nodes require `arrivals` + `served` (optional `errors`), no queue, no capacity, no retry semantics.
...
### Option B — Dependency as a Constraint on a Service (M-0064)
...
- **Allocation rule (M-0064):** proportional to demand per bin across all constrained nodes.
...
This epic depends on the edge semantics contract (M-0007.04) so that dependency load is explicit and validated.
