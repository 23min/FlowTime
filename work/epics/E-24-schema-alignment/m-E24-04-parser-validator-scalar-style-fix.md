...
- **Mirror is mandatory.** If only one validator is fixed, the survey canary will still show residuals sourced from the unfixed side — m-E24-05 cannot close. The milestone wrap requires both fixed.
...
- Unified type or emitter changes — m-E24-02 owns.
...
- m-E24-03 landed (or at least m-E24-02) so the canary baseline reflects the unified schema + emitter state. The m-E24-04 contribution is cleanly attributable to the ParseScalar fix. m-E24-04 is logically independent of m-E24-02 and m-E24-03 and can be run in parallel; the conservative default is serial.
- Prior-art investigation complete (it is — m-E23-01 tracking doc).
