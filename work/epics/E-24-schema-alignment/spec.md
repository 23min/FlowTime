...
- [m-E24-02-unify-model-type](./m-E24-02-unify-model-type.md) — **in-progress (branch `milestone/m-E24-02-unify-model-type`, started 2026-04-25).** Introduce the unified type (`ModelDto` with new `ProvenanceDto`). Rewrite `SimModelBuilder` to emit it directly. Switch Engine intake to parse it directly. Delete `SimModelArtifact` and its satellite types (`SimNode`, `SimOutput`, `SimProvenance`, `SimTraffic`, `SimArrival`, `SimArrivalPattern`). Drop leaked-state fields (`window`, `generator`, top-level `metadata`, top-level `mode`) from emission. Regenerate fixtures and samples under the unified shape. · depends on: m-E24-01
