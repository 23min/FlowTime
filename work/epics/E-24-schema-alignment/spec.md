...
- [m-E24-04-parser-validator-scalar-style-fix](./m-E24-04-parser-validator-scalar-style-fix.md) — **in-progress (started 2026-04-25)** on branch `milestone/m-E24-04-parser-validator-scalar-style-fix`. `ParseScalar` honors `YamlScalarNode.Style` in both `ModelSchemaValidator` and `TemplateSchemaValidator`. Mirror the fix. Dedicated tests cover Plain / SingleQuoted / DoubleQuoted / Literal / Folded styles for scalars that look like bool / int / double. Test-side coercion helper updated or replaced. Pre-state canary: **231 errors** (`ParseScalar` shapes only — 92 `metadata/graph.hidden boolean→string`, 89 `expr integer→string`, 40 `metadata/pmf.expected number→string`, 10 `metadata/pmf.expected integer→string`). Target post-state: **0 errors**. · depends on: m-E24-03 (conservative; can run parallel to m-E24-02 / m-E24-03 if desired)
