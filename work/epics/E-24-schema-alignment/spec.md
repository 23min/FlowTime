...
- [m-E24-01-inventory-and-design-decisions](./m-E24-01-inventory-and-design-decisions.md) — Doc-only milestone. Every field on `SimModelArtifact` and `ModelDefinition` gets a recorded decision (keep / drop / move). The six open questions above are answered. Unified type home, name, and provenance shape are named. No code, no schema changes. · depends on: —
