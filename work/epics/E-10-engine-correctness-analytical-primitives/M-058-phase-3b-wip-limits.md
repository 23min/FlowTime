...
acs:
  - id: AC-1
    title: 'AC-1: WIP limit on ServiceWithBufferNode'
    status: met
  - id: AC-2
    title: 'AC-2: WIP overflow routing'
    status: met
  - id: AC-3
    title: 'AC-3: Model schema updated'
    status: met
  - id: AC-4
    title: 'AC-4: Backpressure pattern documented and tested'
    status: met
  - id: AC-5
    title: 'AC-5: Tests and gate'
    status: met
---
...
### AC-1 — AC-1: WIP limit on ServiceWithBufferNode
...
### AC-2 — AC-2: WIP overflow routing
...
### AC-3 — AC-3: Model schema updated

**AC-3: Model schema updated.** `model.schema.yaml` and `NodeDefinition` support `wipLimit` (number or string series reference) and `wipOverflow` (string node ID or `"loss"`). Template schema updated if applicable.
### AC-4 — AC-4: Backpressure pattern documented and tested

**AC-4: Backpressure pattern documented and tested.** A doc at `docs/architecture/backpressure-pattern.md` describes the SHIFT-based backpressure approach as a first-class modeling pattern. An integration test demonstrates upstream throttling via t-1 queue feedback.
### AC-5 — AC-5: Tests and gate

**AC-5: Tests and gate.** Tests cover: WIP clamping (queue capped at limit), overflow to loss (default), overflow to DLQ node, cascading overflow, time-varying wipLimit (series), backpressure via SHIFT. Full test suite green. Determinism test updated.
## Technical Notes
