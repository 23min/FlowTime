...
**Optimize placeholder replaced.** The `optimize` tab panel in `ui/src/routes/analysis/+page.svelte` renders live content (not the M-041 "coming in M-042" stub). `TAB_INFO` copy for Optimize stands as-is — "convergence history" now accurately describes what the UI renders.
