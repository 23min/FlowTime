...
acs:
  ...
  - id: AC-3
    ...
    status: met
  - id: AC-4
    title: Dark-mode audit across all E-21 surfaces
    status: met
  - id: AC-5
...
### AC-4 — Dark-mode audit across all E-21 surfaces
...
**Elevation audit + token normalization.** Single pass over the shadow / border / layering tokens used across the E-21 surfaces. Catalogue the current uses (border-only, `shadow-sm`, `shadow-md`, custom `box-shadow`, ring-1, …) in the tracking doc; pick a small canonical set (recommend: flat / `shadow-sm` for cards / `shadow-md` for popovers and dropdowns / no shadow on the topology canvas); apply consistently. Out of scope: introducing new elevation tokens; this is a normalization-to-existing pass.
### AC-8 — validation-panel.svelte:157 cosmetic collapse

**`validation-panel.svelte:157` cosmetic collapse.** Replace `{#if row.kind === 'node'}{row.key}{:else}{row.key}{/if}` with `{row.key}`. Verify no behavioural change via the existing M-044 vitest tests on the panel.
