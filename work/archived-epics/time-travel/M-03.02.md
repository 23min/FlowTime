...
**Status:** ✅ Complete (Telemetry capture + Sim orchestration)  
...
### Completion Summary
- CLI tooling (`flowtime telemetry capture` / `bundle`), Sim orchestration scripts, and integration tests landed on the `time-travel-m3` branch (see milestone tracking file).
- Golden capture bundles and telemetry state responses are checked in; provenance metadata flows through to `/state` endpoints.
- Live ADX ingestion is explicitly deferred to a future milestone (TBD). Capture tooling remains the supported path until prioritised.
...
2. Template orchestration that leverages FlowTime-Sim’s `/templates/{id}/generate` contract to produce validated `model.yaml` outputs, keeping ownership with Sim.
...
- [x] Provenance metadata links captured manifest, template-generated model metadata, and canonical artifact bundle.
- [x] Warning and provenance metadata propagate into API responses via existing canonical writer paths.
- [x] Golden fixtures checked in for captured bundles and instantiated template bundles.
- [x] Captures per-node CSV files for arrivals, served, queue depth, errors, and optional external demand directly from `RunArtifactWriter` outputs.
- [x] Produces `manifest.json` capturing `window`, `grid`, `files[]`, `warnings[]`, and `provenance` (run id, template id, hash, checksum) matching canonical layout.
- [x] Supports optional gap/sparsity injection with machine-readable warning codes (`data_gap`, `nan_fill`, etc.) for telemetry-mode validation.
- [x] Provides CLI entry point `flowtime telemetry capture --run-dir data/runs/order-system --output ./out/order-system-telemetry` with dry-run mode describing planned outputs.
- [x] Documents how captured bundles align with future ADX manifests so the ingestion path can later swap to live telemetry without structural changes.
...
**Description:** Leverage FlowTime-Sim’s template runtime to instantiate canonical models while keeping ownership within Sim (current schema supports scalar primitives and arrays; future template enhancements remain Sim’s responsibility).

**Acceptance Criteria:**
- [x] Template schema reference published under `docs/schemas/template.schema.json` (synced from FlowTime-Sim) with authoring guidance for Engine consumers.
- [x] Orchestration scripts/CLI call FlowTime-Sim `/templates/{id}/generate` (or CLI equivalent) to obtain `model.yaml` and provenance metadata (e.g. `scripts/time-travel/run-sim-template.sh`).
- [x] Parameter bundles support Sim’s declared parameter types (scalar primitives and arrays) with explicit defaults and required flags documented for UI orchestration.
- [x] Instantiated output feeds directly into `RunArtifactWriter` to emit canonical `model.yaml`, `metadata.json`, and `provenance.json`.
- [x] Manual steps are documented for environments where Sim service and Engine CLI run separately, keeping concerns isolated.

#### FR3: Capture ↔ Template Integration Workflow
**Description:** Ensure captured telemetry bundles flow through Sim templates into canonical bundles that `/state` can replay.

**Acceptance Criteria:**
- [x] Documented workflow demonstrates captured telemetry directory wired into template parameters (e.g., `telemetry_dir`) without manual editing.
- [x] Integration tests cover end-to-end path: capture telemetry bundle from Engine run → call Sim `/templates/{id}/generate` with references to bundle → run `RunArtifactWriter` → call `StateQueryService`.
- [x] Provenance metadata links captured manifest, template-generated model metadata, and canonical artifact bundle.
- [x] Warning and provenance metadata propagate into API responses via existing canonical writer paths.
- [x] Golden fixtures checked in for captured bundles and instantiated template bundles.
...
**Target:** Capture telemetry windows up to 288 bins × 50 nodes with median runtime ≤ run duration and peak memory ≤250 MB.
...
### Schema References
- `docs/schemas/time-travel-state.schema.json` — response schema for `/state` consumers, ensuring loader outputs remain compatible.
- `docs/schemas/template.schema.json` — new schema defining template structure, parameter metadata, and include rules.
- `docs/schemas/telemetry-manifest.schema.json` — new schema validating captured telemetry manifests.
...
**Success Criteria:**
- [x] Capture CLI reproduces canonical bundles from deterministic runs.
- [x] Manifests capture warnings and provenance metadata.
- [x] Golden fixtures added for regression coverage.
...
**Success Criteria:**
- [x] Sim `/generate` invocation produces models referencing captured telemetry without manual edits.
- [x] Canonical bundles include matching template + capture provenance metadata.
- [x] Documentation reviewed with Sim and Engine owners.

### Phase 3: End-to-End Validation & Docs
**Goal:** Validate the capture → template → `/state` workflow and capture operator guidance.

**Tasks:**
1. Add integration tests covering capture → Sim → `RunArtifactWriter` → `StateQueryService`.
...
**Success Criteria:**
- [x] End-to-end test passes using captured bundles and Sim templates.
- [x] `/state` reads captured bundles without manual edits.
- [x] Documentation reviewed and linked from roadmap + milestone tracker.
...
### Integration Tests
- Capture → Sim `/generate` → `RunArtifactWriter` path for each example template.
