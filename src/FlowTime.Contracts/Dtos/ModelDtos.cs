...
/// <summary>
/// Grid definition specifying time bins and duration. The grid start timestamp is the
/// canonical wire field <c>start:</c> — m-E24-01 ratified A1 (delete the prior
/// <c>LegacyStart</c> shim) and A2 (rename the C# property from <c>StartTimeUtc</c> to
/// <c>Start</c> so the camelCase convention emits/reads <c>start:</c> natively without an
/// alias). Production templates and the canonical schema always use <c>start:</c>;
/// <c>startTimeUtc:</c> never appeared on the wire — it was a fictional emission target
/// generated by the camelCase convention from the prior C# name.
/// </summary>
...
public sealed class TopologySemanticsDto
{
    ...
    // m-E24-02 step 4: nullable to mirror runtime TopologyNodeSemanticsDefinition.Errors
    // and avoid emitting empty `errors: ''` for sink nodes (which legitimately don't
    // declare errors). Existing wire format omits the field entirely for sinks; the
    // null-default + OmitNull semantics on the SerializerBuilder preserves that.
