...
- **E-21** Svelte Workbench & Analysis Surfaces (`work/epics/E-21-svelte-workbench-and-analysis/spec.md`) — **in-progress**
  ...
  - **m-E21-05** (in-progress, branch `milestone/m-E21-05-optimize`, started 2026-04-22): Optimize Surface — Wire `/analysis` Optimize tab to live `/v1/optimize` (N-param Nelder-Mead under bounds). Consumes shared `AnalysisResultCard` + `ConvergenceChart` + `interval-bar-geometry` from m-E21-04; adds `flowtime.optimize(...)` client method; sibling `optimize-helpers.ts` module for form validation / per-param table helpers. Per-param result table with mini range bars showing where each optimized value landed inside its bound. Backend trace already landed in m-E21-04 commit `29ac3e9` — no backend work this milestone.
