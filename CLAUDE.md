...
- **Immediate next step:** merge `milestone/m-E19-04-blazor-support-alignment` into `epic/E-19` (wrap complete, awaiting approval), then assess `epic/E-19 → main` merge now that all four E-19 milestones are complete and every epic success criterion is checked; E-10 Phase 3 remains ready to resume (`work/epics/epic-roadmap.md`)
...
- **E-19** Surface Alignment & Compatibility Cleanup (`work/epics/E-19-surface-alignment-and-compatibility-cleanup/spec.md`) — **all four milestones completed; epic→main merge pending**
  - **Branch topology:** `epic/E-19` (integration branch, m-E19-02 + m-E19-03 merged); `milestone/m-E19-01-supported-surface-inventory`, `milestone/m-E19-02-sim-authoring-and-runtime-boundary-cleanup`, and `milestone/m-E19-03-schema-template-example-retirement` (frozen pointers, merged); `milestone/m-E19-04-blazor-support-alignment` (wrap complete, awaiting merge into `epic/E-19`); `epic/E-19 → main` merge awaiting approval
  ...
  - **Completed (m-E19-03):** deprecated `binMinutes` YAML authoring shape retired from `TemplateServiceImplementations.cs` demo generators, `run-example.json` fixture, `Cli/Program.cs` verbose label, and `ParameterConversionIntegrationTests.cs` parameter dict keys; `whitepaper.md` + `retry-modeling.md` YAML examples rewritten to `binSize`/`binUnit` (Little's Law math notation preserved via `m-E19-03:allow-binminutes-notation` inline marker); three schema-migration example YAMLs archived to `examples/archive/`; stale `docs/ui/template-integration-spec.md` archived to `docs/archive/ui/`; catalog-stale phrasing cleaned from `UI.md`, `contracts.md`, `engine-capabilities.md`; grep-guard script (`scripts/m-E19-03-grep-guards.sh`, 11/11 passing). Bundled into 5 commits.
  - **Completed (m-E19-04):** stale `FlowTime.UI` Sim client wrappers `RunAsync`/`GetIndexAsync`/`GetSeriesAsync` (rows 64, 65) deleted from `IFlowTimeSimApiClient` + `FlowTimeSimApiClient` + `FlowTimeSimApiClientWithFallback`; `SimRunResponse` DTO deleted; `FlowTimeSimService.RunApiModeSimulationAsync` rewired onto supported `simClient.CreateRunAsync(new RunCreateRequestDto(...))` orchestration path; `FlowTimeSimService.GetRunStatusAsync` rewired onto `apiClient.GetRunIndexAsync` Engine query; `SimResultsService` simplified to Engine API only (dropped `isEngineRun` branch and `IFlowTimeSimApiClient` ctor dep); `SimulationResults.razor` download-URL branch collapsed onto canonical Engine URL with demo-mode info notification; `ResultsUrl` dead sentinel field removed from `SimulationRunResult`; orphaned helpers `GenerateSimulationYamlAsync`/`TranslateToSimulationSchema`/`ConvertRequestToApiParameters` + `YamlDotNet` usings deleted. Row 63 Blazor Sim client surface and rows 66/67 Svelte clients audited for drift — no drift found. Grep-guard script (`scripts/m-E19-04-grep-guards.sh`, 11/11 passing, with fail-fast `command -v rg` improvement over m-E19-03). Final test count 1246 passed / 9 skipped (−4 from baseline due to deleted tests exercising deleted helpers). Bundled into 4 commits (status sync / Bundle A / Bundle C / wrap).
  - **Next:** merge `milestone/m-E19-04-blazor-support-alignment` into `epic/E-19`, then assess `epic/E-19 → main` merge — all 9 E-19 success criteria are now checked
