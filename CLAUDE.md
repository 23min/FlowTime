...
**Active focus:** m-E23-03-delete-model-validator — delete `src/FlowTime.Core/Models/ModelValidator.cs`, move `ValidationResult` (lines 202-214 of that file) to its own `src/FlowTime.Core/Models/ValidationResult.cs` keeping namespace `FlowTime.Core`, assert `grep -rn "ModelValidator\b" --include="*.cs"` returns zero live hits (only historical comments survive), archive E-23 epic to `work/epics/completed/`. m-E23-02 closed 2026-04-26 (3 production sites + 28 test calls migrated; real `ProvenanceService.StripProvenance` round-trip bug fixed; +16 tests; suite 1862/0/9). m-E23-01 closed 2026-04-26 (94 rules audited, 16 schema-add edits, 5-arm `oneOf` restructure, silent-error fallback, 12 adjunct methods, 26-test regression catalogue).
...
- **E-23** Model Validation Consolidation (`work/epics/E-23-model-validation-consolidation/spec.md`) — **in-progress (m-E23-01 + m-E23-02 completed 2026-04-26; m-E23-03 next — pure deletion)** — E-24 Schema Alignment closed; E-23 spirit reframed.
