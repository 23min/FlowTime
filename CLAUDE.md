...
- **E-24** Schema Alignment (`work/epics/E-24-schema-alignment/spec.md`) — **complete (2026-04-25)** — all five milestones landed on `epic/E-24-schema-alignment`. Epic interrupted E-23 at m-E23-01 on 2026-04-24; internal planning ratified **Option E (unify)** on 2026-04-24 per `D-2026-04-24-037`; closed two days later. **m-E24-01 Inventory & Design Decisions complete (commit `c43e8c0`)**; **m-E24-02 Unify Model Type complete (commits `131dd35` / `c4064bf` / `3806097` / `dece22f`); m-E24-03 Schema Unification complete (commit `d6a3263`); m-E24-04 Parser/Validator Scalar-Style Fix complete (commit `a7c984f`); m-E24-05 Canary Green + Hard Assertion complete (2026-04-25)**. Integration branch ready for merge to `main` (next deployer-scope action) and subsequent archive to `work/epics/completed/E-24-schema-alignment/`. Post-m-E24-02 cleanup wave merged into epic 2026-04-25 (commit `5a2a31d`): three E-24 wake gaps resolved (`ProvenanceEmbedder` deleted, `GridDefinition.StartTimeUtc` → `Start`, Template `Legacy*` aliases dropped). Closure logged in `D-2026-04-25-038`.
  - **Goal:** Unify FlowTime's post-substitution model representation. One C# type. One YAML schema. One validator. `SimModelArtifact` **deleted**; Sim builds the unified type directly; Engine parses it directly. `Template` (authoring-time) stays distinct. Forward-only — no migration of stored bundles. camelCase throughout.
  - **Why now:** The two-type split (`SimModelArtifact` vs `ModelDefinition`) is accidental drift, not a designed interface. `SimModelArtifact` was introduced in commit `ce9ec9e` (Oct 2025) shaped to the template schema, not to `model.schema.yaml` which predated it. No C# type boundary exists — only a YAML string — and that YAML string fails the Engine's own schema in every shipped template (m-E23-01 canary survey, agent `a07d52c12dcaf3538`, 726 validator errors across 16 divergence shapes). Each feature addition since has paid a duplication tax. Only `provenance` has genuine dual concern, and both sides already agree on the keys — they just disagree on casing. Investigation: agent `a5aa3dfe26394aff5`.
  - **Option A rejected:** The initial draft proposed preserving two types with a projection layer. Rejected as preserving accidental drift; converts the leak into a designed feature; accepts the ongoing duplication tax.
  - **Planned milestones (5):** m-E24-01 Inventory & Design Decisions — **complete (2026-04-25, commit `c43e8c0`)**; doc-only; unified type home (`FlowTime.Contracts`), name (`ModelDto`), nested 7-field camelCase provenance shape, six open questions ratified (Q1–Q6 + A1–A5) · m-E24-02 Unify Model Type — **complete (2026-04-25, commits `131dd35` / `c4064bf` / `3806097` / `dece22f`)**; all 14 ACs landed; `ModelDto`+`ProvenanceDto` introduced; `SimModelArtifact` + 6 satellites deleted; `SimModelBuilder` rewritten; Engine intake unified; leaked-state fields dropped from emission; YamlDotNet bumped to 17.0.1 across 6 projects; AC8 residual split: 587 → m-E24-03, 89 → m-E24-04, 0 → m-E24-02 regressions; 1,755 tests passing; 3 follow-up gaps filed (`ProvenanceEmbedder`, `GridDefinition.StartTimeUtc` rename, Template `Legacy*`) · m-E24-03 Schema Unification — **complete (2026-04-25, commit `d6a3263`)**; all 14 ACs landed; `docs/schemas/model.schema.yaml` rewritten top-to-bottom (927 → 1059 lines) against unified `ModelDto`; nested 7-field camelCase provenance; `grid.start`/`nodes[].metadata` declared, `nodes[].source` absent, `outputs[].as` optional; consumer citations on every property; canary 676 → 231 errors (schema-rewrite shapes 587 → 0; remaining 231 are m-E24-04 ParseScalar residuals — same defect class on newly-reachable inner paths); 1,750 tests passing · m-E24-04 Parser/Validator Scalar-Style Fix — **complete (2026-04-25, commit `a7c984f`)**; all 9 ACs landed; mirrored `ParseScalar` `ScalarStyle.Plain` guard in both `ModelSchemaValidator` and `TemplateSchemaValidator`; test-side coercion helper rewritten to delegate through the same rule; sibling `QuotedAmbiguousStringEmitter` plugged into `TemplateService.CreateYamlSerializer` next to `FlowSequenceEventEmitter` for round-trip symmetry (D-m-E24-04-03 / ADR-E-24-05); canary **231 → 0 errors** at `ValidationTier.Analyse` across 12 templates; histogram empty post-fix; +64 new tests (18 Engine ParseScalar + 17 Sim ParseScalar + 29 emitter round-trip); full `.NET` suite green · m-E24-05 Canary Green + Hard Assertion — **in-progress (started 2026-04-25)**; `Survey_Templates_For_Warnings` rewritten with phase-1 hard `val-err == 0` assertion + phase-2 graceful-skip live-API survey; regression-catching verified by transient un-wire of `QuotedAmbiguousStringEmitter` (canary fails with 231 errors across all 12 templates exactly matching the m-E24-04 pre-fix histogram, then returns to green on restore); full `.NET` suite green (1,749 passed / 9 skipped pre-existing / 0 failed); all three AC4 grep audits clean (snake_case keys / `SimModelArtifact` / satellite types — zero hits in `src/` and `tests/`); `docs/schemas/model.schema.md` provenance section rewritten to canonical 7-field nested camelCase form with historical note; E-23 status flipped from `paused` to `ready-to-resume`; `D-2026-04-25-038` logs E-24 close.
  ...
  - **Integration branch:** `epic/E-24-schema-alignment` (created from `main` 2026-04-24).
  - **Out of scope (firm):** `ModelValidator` deletion (stays with E-23 m-E23-03), UI work (Blazor + Svelte), new validator features (LSP, line/column mapping), deprecated-field reintroduction, migration windows / aliases, bundle migration (forward-only — existing stored bundles obsolete), `Template` (authoring-time) redesign, E-15 runtime implementation (E-24 decides `nodes[].source` contract so E-15 starts aligned).
  - **Input material:** Uncommitted m-E23-01 artefacts (three schema edits for `grid.start` / `nodes[].metadata` / `nodes[].source`; two `work/gaps.md` entries; m-E23-01 tracking doc) stashed on branch `milestone/m-E23-01-schema-alignment`. Treated as input to m-E24-01 design-decision review under the unification framework, not as commitment.
...
- **E-23** Model Validation Consolidation (`work/epics/E-23-model-validation-consolidation/spec.md`) — **ready-to-resume (2026-04-25)** — E-24 Schema Alignment closed.
  - All five E-24 milestones landed on `epic/E-24-schema-alignment`. m-E23-02 (call-site migration) and m-E23-03 (`ModelValidator` delete) are now byte-trivial mechanical cleanup — `ModelSchemaValidator`, the schema, Sim's emitter, and the Engine's reader literally share a single unified type definition (`ModelDto` / `ProvenanceDto` in `FlowTime.Contracts`). Reentry needs a status-reconciliation pass on the `milestone/m-E23-01-schema-alignment` branch's stashed input material before m-E23-02 starts (most input has been absorbed by E-24).
  - **Milestone status post E-24 close:** m-E23-01 Schema Alignment — **largely absorbed by E-24** (`model.schema.yaml` rewritten by m-E24-03; canary committed and now hard-asserts under m-E24-05); m-E23-02 Call-Site Migration — **ready** (E-24 prerequisite cleared); m-E23-03 Delete `ModelValidator` — **ready** (waits on m-E23-02).
  - **Unblocks (after resume + close):** m-E21-07 Validation Surface (Svelte) — consumes the consolidated `ModelSchemaValidator`.
  - **Integration branch:** `epic/E-23-model-validation-consolidation` (preserved; will receive E-23 work once E-24 closes).
