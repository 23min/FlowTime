...
- **E-21** Svelte Workbench & Analysis Surfaces (`work/epics/E-21-svelte-workbench-and-analysis/spec.md`) — **in-progress**
  ...
  - **m-E21-06** (in-progress, started 2026-04-23, branch `milestone/m-E21-06-heatmap-view`): Heatmap View — nodes-x-bins grid as a sibling of topology under `/time-travel/topology` with typed `<ViewSwitcher>` (inline views, no registry per ADR-m-E21-06-01), shared view-state store (`view-state.svelte.ts`), shared full-window 99p-clipped color-scale normalization (topology straight-swaps from per-bin per ADR-m-E21-06-02 — no `?normalization=per-bin` escape hatch), shared-toolbar `[ Operational | Full ]` node-mode toggle reaching Blazor parity (AC15, adds optional `mode` param on `getStateWindow`). 15 ACs; zero backend work (`state_window` suffices). Testing target: 13 Playwright + 5 vitest pure-logic suites with line-by-line branch audit. Baseline at start: 501 ui-vitest, 304 dag-map, .NET green. Confirmations captured: (1) ADR-02 straight swap, (2) Playwright #11 bucket-only, (3) AC12 homework → gaps, (4) topo-sort = reuse-or-Kahn at implementation time.
