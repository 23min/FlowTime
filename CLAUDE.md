...
- **E-11** Svelte UI — **completed (2026-05-02).** All in-tree milestones done (M-061, M-062). M5 Inspector and M8 Polish absorbed into E-21 Svelte Workbench. M7 Dashboard deferred. M-062 Run Orchestration verified end-to-end via new Playwright spec (`tests/ui/specs/svelte-run-orchestration.spec.ts`); discovered + closed Sim-side defect G-034 (orchestrator silently demoted `dryRun:true` when a deterministic run already existed).
