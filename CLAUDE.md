...
**Active focus:** m-E23-03-delete-model-validator (next) — delete `src/FlowTime.Core/Models/ModelValidator.cs`, move `ValidationResult` to its own file, assert grep returns zero callers, archive E-23. m-E23-02 closed 2026-04-26 (3 production sites + 28 test calls migrated to `ModelSchemaValidator.Validate`; `TimeMachineValidator` redundant-delegation block removed; real `ProvenanceService.StripProvenance` round-trip bug fixed via YamlStream surgical removal; 16 net new tests including a watertight `/v1/run`-level integration regression; suite 1862/0/9). m-E23-01 closed 2026-04-26 (94 rules audited, 16 schema-add edits, 5-arm `oneOf` restructure, silent-error fallback, 12 adjunct methods, 26-test regression catalogue).
