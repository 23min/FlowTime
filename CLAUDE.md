...
- **E-24** Schema Alignment (`work/epics/E-24-schema-alignment/spec.md`) — **in-progress** — epic interrupting E-23 at m-E23-01 (2026-04-24). Internal planning ratified **Option E (unify)** on 2026-04-24 per `D-2026-04-24-037`; 5-milestone plan below. **m-E24-01 Inventory & Design Decisions complete (2026-04-25, commit `c43e8c0`)**; **m-E24-02 Unify Model Type in-progress (branch `milestone/m-E24-02-unify-model-type`, started 2026-04-25)**.
  ...
  - **Planned milestones (5):** m-E24-01 Inventory & Design Decisions — **complete (2026-04-25, commit `c43e8c0`)**; doc-only; unified type home (`FlowTime.Contracts`), name (`ModelDto`), nested 7-field camelCase provenance shape, six open questions ratified (Q1–Q6 + A1–A5) · m-E24-02 Unify Model Type — **in-progress (branch `milestone/m-E24-02-unify-model-type`, started 2026-04-25)**; introduce `ModelDto` + new `ProvenanceDto`; rewrite `SimModelBuilder` to emit directly; switch Engine intake; delete `SimModelArtifact` + satellites `SimNode`/`SimOutput`/`SimProvenance`/`SimTraffic`/`SimArrival`/`SimArrivalPattern`; drop leaked-state fields `window`/`generator`/top-level `metadata`/top-level `mode`; regenerate fixtures · m-E24-03 Schema Unification (rewrite `docs/schemas/model.schema.yaml` to describe the unified type; camelCase provenance; rewrite `docs/schemas/README.md`; audit `docs/architecture/`) · m-E24-04 Parser/Validator Scalar-Style Fix (`ParseScalar` honors `YamlScalarNode.Style` in both `ModelSchemaValidator` and `TemplateSchemaValidator`; mirrored fix + test helper) · m-E24-05 Canary Green + Hard Assertion (`TemplateWarningSurveyTests` asserts `val-err=0`; E-23 resume gate).
