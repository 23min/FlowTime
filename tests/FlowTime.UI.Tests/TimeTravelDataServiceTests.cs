...
public class TimeTravelDataServiceTests
{
    ...
    public async Task GetStateWindowAsync_ReturnsFailure_WhenEndBinBeforeStart()
    {
        ...
        var result = await service.GetStateWindowAsync("run", 5, 4, mode: null, ct: CancellationToken.None);
    ...
    public async Task GetStateWindowAsync_ReturnsPayload_OnSuccess()
    {
        ...
        var result = await service.GetStateWindowAsync("run", 0, 1, mode: null, ct: CancellationToken.None);
