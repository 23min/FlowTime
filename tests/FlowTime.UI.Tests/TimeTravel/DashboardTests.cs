...
public class DashboardTests
{
    ...
    public async Task MetricsClient_FallsBackToStateWindow_WhenEndpointMissing()
    {
        ...
        var data = new StubTimeTravelDataService
        {
            StateWindowHandler = (_, _, _, _, _, _) => Task.FromResult(ApiCallResult<TimeTravelStateWindowDto>.Ok(stateWindow, 200))
        };
    ...
    private sealed class StubFlowTimeApiClient : IFlowTimeApiClient
    {
        ...
        public Func<string, int, int, string?, bool, CancellationToken, Task<ApiCallResult<TimeTravelStateWindowDto>>> StateWindowHandler { get; set; } =
            (_, _, _, _, _, _) => Task.FromResult(ApiCallResult<TimeTravelStateWindowDto>.Fail(404, "not implemented"));
        ...
        public Task<ApiCallResult<TimeTravelStateWindowDto>> GetRunStateWindowAsync(string runId, int startBin, int endBin, string? mode = null, bool includeEdges = false, CancellationToken ct = default)
            => StateWindowHandler(runId, startBin, endBin, mode, includeEdges, ct);
