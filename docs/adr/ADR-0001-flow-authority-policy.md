...
The question reached the engine through gap [G-032](../../work/gaps/G-032-transportation-basic-regressed-edge-flow-mismatch-incoming-3-after-e-24-unification.md), which surfaced a latent inconsistency in nine shipped templates after E-24's schema unification: edge weights and consumer-side `expr` arithmetic disagreed about how a producer's outflow should split. G-032 framed the resolution as a choice between three options:
...
- **Footprint analysis.** M-066 walked all three G-032 options against representative templates and counted the diff. The cost evidence and the purity argument agree: option 1 (edge weights win) wins on engineering cost as well as first principles. The full preserved analysis lives in M-066 under [Footprint analysis (M-066's working artefact)](../../work/epics/E-25-engine-truth-gate/M-066-edge-flow-authority-decision.md#footprint-analysis-m-066s-working-artefact) — see the "Option footprint summary" table and the "Read of the evidence" subsection for the bucketed engine-LOC magnitudes and per-template diff estimates that informed this decision.
...
This ADR names *what* each layer enforces. The implementation — schema constraints, compile-time errors, analyser warning families — lands in M-069.

## Consequences

### Immediate consequences

- **G-032 is resolved at the design layer.** The three options it lists are answered: option 1 (edge weights win) is the right answer for class 3, which is what all nine affected shipped templates instantiate. Option 2 is rejected on first principles; option 3 is rejected on structural-redundancy grounds. G-032's status moves to `addressed` with a reference to this ADR (per M-066 AC-10); the gap is fully closed only once the engine + template alignment milestone (M-067) lands.
- **Class-3 authoring guidance is folded into M-067.** The taxonomy clarifies that edge weights are a first-class authoring pattern alongside the router node. M-067 (Engine + Template Alignment) revisits authoring guidance as part of its surface; no separate gap is required for this enhancement.
...
- **M-069 (Schema + Compile + Analyse Enforcement)** carries the enforcement implementation named under [Enforcement points](#enforcement-points). The ADR pins *what* each layer enforces; M-069 lands the schema constraints, compile-time errors, and analyser warning families.
- **M-067 (Engine + Template Alignment)** edits the nine G-032-affected templates to encode their class-3 splits as edge weights, resets the `ExpectedRunWarnings` baseline accordingly, and folds in class-3 first-class authoring guidance.
- **M-068 (Golden-Output Canary)** ratifies the post-alignment baseline as the canary fixture.

### User-facing consequences

- **Templates that encoded class-3 splits in consumer `expr` arithmetic must migrate to edge weights.** The migration is mechanical for the shipped set (nine templates, walked under M-066's footprint analysis). User-authored templates outside the shipped set may need similar migration; tooling for that is out of E-25 epic scope.
- **The engine's gates will reject peer-relative consumer arithmetic and ambiguous fan-out authority once M-069 lands.** Authors will see schema errors, compile-time errors, or analyser warnings depending on the violation. The taxonomy doc is the reference a future template author reads to know which class their model belongs in.

### Architectural consequences

- **Conservation invariant is single-direction truth for class 3.** With edge weights normative on the producer side, `arrivals(t) == sum(incomingEdges_after_lag)(t)` becomes the conservation gate without ambiguity. This simplifies reasoning for E-15 telemetry replay (replay substitutes telemetry-observed `flowVolume`, not consumer arithmetic) and E-22 model fit (the routing surface to fit against is unambiguous).
- **Future class-2 surface is unconstrained by this ADR.** [G-038](../../work/gaps/G-038-class-2-capacity-aware-allocator-deferred-from-m-066-flow-authority-policy.md) names the deferred capability and the modeling cases that motivate it; the allocator-node specification is open and will be designed under that gap's carrier milestone, not this ADR.
...
- **Milestone:** [M-066 — Flow-Authority Policy Spike](../../work/epics/E-25-engine-truth-gate/M-066-edge-flow-authority-decision.md). The milestone that authored this ADR. See its [Footprint analysis (M-066's working artefact)](../../work/epics/E-25-engine-truth-gate/M-066-edge-flow-authority-decision.md#footprint-analysis-m-066s-working-artefact) section for the engineering-cost evidence cited above, and its [Doc-sweep classification (M-066's AC-3 artefact)](../../work/epics/E-25-engine-truth-gate/M-066-edge-flow-authority-decision.md#doc-sweep-classification-m-066s-ac-3-artefact) section for the per-document classification against this policy.
- **Originating gap:** [G-032 — `transportation-basic` regressed: `edge_flow_mismatch_incoming` × 3 after E-24 unification](../../work/gaps/G-032-transportation-basic-regressed-edge-flow-mismatch-incoming-3-after-e-24-unification.md). The investigation that surfaced the latent class-3 inconsistency in the shipped template set.
- **Class-2 deferred capability:** [G-038 — Class-2 capacity-aware allocator (deferred from M-066 flow-authority policy)](../../work/gaps/G-038-class-2-capacity-aware-allocator-deferred-from-m-066-flow-authority-policy.md). The deferred-follow-up gap filed under M-066 AC-7.
- **Epic:** [E-25 — Engine Truth Gate](../../work/epics/E-25-engine-truth-gate/epic.md). The epic that owns the policy, the enforcement implementation (M-069), the engine + template alignment (M-067), and the golden-output canary (M-068).
