# M-03.04 — Run Orchestration APIs
...
- **Acceptance Criteria:**
  - [ ] Request with template id + telemetry bundle path orchestrates capture (if required), template generation, and canonical artifact writing.
  - [ ] Response returns run metadata (`runId`, `mode`, `template`, `schema`, `provenanceHash`, warning summary) and links to `/state`, `/state_window`.
  - [ ] Validations enforce presence of window/timeline info and template contracts; errors use documented codes.
  - [ ] Supports dry-run mode (`?dryRun=true`) that reports planned operations without writing files.

#### FR2: Run Listing
- **Description:** `GET /v1/runs` returns a paginated collection of run summaries sorted by creation time.
- **Acceptance Criteria:**
  - [ ] Includes basic fields: `runId`, `mode`, `templateId`, `templateTitle`, `startUtc`, `duration`, `warnings`.
  - [ ] Supports filters by `mode`, `templateId`, and `hasWarnings`.
  - [ ] Pagination parameters (`page`, `pageSize`) validated; defaults documented.

#### FR3: Run Detail
- **Description:** `GET /v1/runs/{runId}` returns enriched metadata (storage paths, provenance hashes, available telemetry sources).
- **Acceptance Criteria:**
  - [ ] Mirrors the metadata envelope consumed by `/state` (`StateMetadata`) and includes any bundle manifest warnings.
  - [ ] Returns `404` with structured error when run is missing.
  - [ ] Indicates readiness for `/state` via `canReplay` flag (true when artifacts exist).

#### FR4: Orchestration Service Layer
- **Description:** Introduce reusable orchestration services invoked by API and CLI.
- **Acceptance Criteria:**
  - [ ] Encapsulates capture, template generation, and bundling with cancellation support.
  - [ ] Emits structured logs (start, progress, completion, warnings) for observability.
  - [ ] CLI commands reuse the service layer to avoid drift.

### Non-Functional Requirements
- **NFR1 (Performance):** Single run orchestration completes within 2× the CLI baseline (document baseline from existing `site` telemetry bundle).
...
### Unit Tests
- `RunRequestValidatorTests` — validates required fields, window bounds, template existence.
- `RunOrchestrationServiceTests` — dry-run, deterministic id, warning propagation.
- `RunRepositoryTests` — listing filters, pagination, metadata extraction.

### Integration Tests
- `RunApiTests.CreateTelemetryRun_ReturnsMetadata()` — orchestrates a run using fixture bundle.
- `RunApiTests.ListRuns_FiltersByTemplateAndMode()` — seeds indexed runs and asserts filtering.
- `RunApiTests.RunDetail_MirrorsStateMetadata()` — ensures metadata matches `/state`.
- `RunApiTests.CreateRun_InvalidTemplate_Returns422()` — exercises error handling.
