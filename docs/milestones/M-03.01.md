...
- Refresh Engine artifact persistence: update `RunArtifactWriter` so `/v1/run` emits the canonical `model/metadata/provenance` layout used by SIM, enabling `/state` replays without manual artifact fixes.
