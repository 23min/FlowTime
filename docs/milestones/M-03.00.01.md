...
- FlowTime.Expressions assembly now exposes shared SHIFT self-reference detection and Engine consumes the helper; additional validation parity coverage still pending.
- Manual `.http` request payload (`src/FlowTime.API/FlowTime.API.http`) refreshed to exercise topology fixture data while verifying `/state` and `/state_window`.
