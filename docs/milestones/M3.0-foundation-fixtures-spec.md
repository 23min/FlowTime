...
**Behavior:**
- If `startTime` present → absolute time model
- If `startTime` absent → relative time model (backward compatible)
...
topology:
  nodes:
    - id: "OrderService"
      initialCondition:
        queueDepth: 50  # Orders in queue at bin 0
```

**Validation:**
- If `initialCondition.queueDepth` present → enforce at bin 0
- If absent → assume queue starts empty (queueDepth = 0)
- Conservation: `queueDepth[0] = initialCondition.queueDepth + arrivals[0] - served[0] - errors[0]`
