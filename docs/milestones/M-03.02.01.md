...
**Target:** Enable `/v1/runs` to generate run bundles (per-series CSVs plus an `aggregates/` placeholder) using **simulation mode** without requiring telemetry capture input. The milestone introduces a simulation-only orchestration pipeline, returning canonical artifacts (`run.json`, `manifest.json`, `series/index.json`) and structured metadata so downstream APIs and the UI can consume newly generated runs immediately.

> **Terminology update:** Earlier drafts called these outputs "gold bundles." The new term **run bundle** better reflects the canonical artifact set (per-series CSVs + optional aggregates).

---

## Overview

M-03.00 through M-03.02 laid the groundwork for time-travel: canonical run artifacts, mode-aware validation, and telemetry capture/bundling. However, `/v1/runs` currently assumes telemetry-backed runs. Simulation mode—where FlowTime.Sim generates synthetic telemetry directly from templates—remains CLI-only. UI milestones (UI-M-03.12) and operator workflows now depend on a REST surface that can produce a run bundle without telemetry inputs.
...
- `POST /v1/runs` with `mode=simulation` generates a run bundle from the specified template and parameters.
...
- **NFR1 (Performance):** Simulation orchestration should complete within 2× the current CLI baseline for the order-system template.
...
- ✅ `RunOrchestrationService` branches on simulation mode, generates FlowTime.Sim series, writes canonical artifacts, emits telemetry manifests, and produces structured logs/metrics (`run_created_total`, `simulation_evaluation_duration_ms`).
- ✅ `/v1/runs` accepts simulation payloads, returns `/state`-aligned metadata, enforces window/topology validation, and surfaces template errors as `400` responses.
- ✅ CLI `flowtime run --template-id <id> --mode simulation` reuses the shared orchestration; telemetry path remains available for capture-backed runs.
- ✅ Integration, golden, and CLI tests cover simulation orchestration; `FlowTime.API.http` now documents simulation samples alongside telemetry flows.
- ✅ Observability automated: unit tests assert metric counters/histogram emission and `RunOrchestrationCompleted` logging for simulation runs.
...
- Follow strict TDD (Red → Green → Refactor) for all changes: write failing tests first before implementing service logic.
...
- Maintain TDD discipline—add/extend tests that fail before changing API/CLI code, then refactor once green.
...
- Continue Red–Green–Refactor cycle; expand tests before implementation adjustments.
