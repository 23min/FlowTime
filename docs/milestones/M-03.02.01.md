...
M-03.00 through M-03.02 laid the groundwork for time-travel: canonical run artifacts, mode-aware validation, and telemetry capture/bundling. However, `/v1/runs` currently assumes telemetry-backed runs. Simulation mode—where FlowTime.Sim generates synthetic telemetry directly from templates—remains CLI-only. UI milestones (UI-M-03.12) and operator workflows now depend on a REST surface that can produce a gold run without telemetry inputs.
