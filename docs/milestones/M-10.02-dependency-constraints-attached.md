...
- Each constraint has its own arrivals/served signals (from telemetry or derived). If signals are missing, warnings must surface.
...
- Series metadata must flag derived vs explicit sources.
...
### Documentation
- Update `docs/architecture/dependency-constraints/README.md` with Option B semantics.
- Add notes to `docs/reference/engine-capabilities.md` for constraint-limited nodes.

## Decisions Needed

1. **Allocation rule:** proportional to demand vs proportional to capacity vs fixed priority.
