...
- [ ] Orchestration scripts/CLI call FlowTime-Sim `/templates/{id}/generate` (or CLI equivalent) to obtain `model.yaml` and provenance metadata.
...
- [ ] Integration tests cover end-to-end path: capture telemetry bundle from Engine run → call Sim `/templates/{id}/generate` with references to bundle → run `RunArtifactWriter` → call `StateQueryService`.
...
- Expand `FlowTime.CLI` reference to document new capture command and Sim orchestration scripts.
