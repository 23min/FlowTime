...
- **Node kinds**: const (inline values), expr (limited expression set), **serviceWithBuffer** nodes that own queue/backlog semantics (`queueDepth`, arrivals/served/errors, optional `loss`, dispatch schedules), routers, sinks, and **dependency** nodes (arrivals/served/errors only). Template-derived computed nodes are preserved in artifacts. (See [`work/epics/completed/service-with-buffer/service-with-buffer-architecture.md`](../architecture/service-with-buffer/service-with-buffer-architecture.md) for the full contract.)
- **Dependency constraints (Option B) — foundations laid**: services can declare shared constraints (registry) with proportional allocation logic (`ConstraintAllocator`). However, the allocator is **not called during evaluation** — declared constraints are loaded but silently ignored at runtime. Enforcement is pending (see `work/gaps.md`, "Dependency Constraint Enforcement").
...
- **Constraints in state/state_window**: per-node `constraints` series (arrivals/served/errors/latency + derived shortfall) and `constraintStatus` (limited flag per bin). Constraints are series-backed resources, not nodes. Note: constraint data is loaded and exposed in state responses, but **not enforced** during evaluation — the constraint allocator has zero callers in the evaluation pipeline.
