...
- **when** Studio should land relative to E-0015, Telemetry Loop & Parity, and
  E-0022
...
2. explicit "yes, sequence Studio as E-0023" decision logged in
   `work/decisions.md`
3. E-0022's planning finalizes with the narrow-SDK scope (so Studio isn't
   racing a broader E-0022 shape)
4. E-0015's first-dataset milestone has a draft that names the
...
flowchart LR
    ...
    E21["E-0021<br/>Svelte Workbench<br/>&amp; Analysis Surfaces<br/>● in flight (6/8)"]
    E15["E-0015<br/>Telemetry Ingestion<br/>+ sliding-window contract<br/>○ next"]
    PARITY["Telemetry Loop<br/>&amp; Parity<br/>○ next"]
    E22["E-0022<br/>Model Fit + Chunked<br/>+ Pipeline SDK (narrow)<br/>○ next"]
    E23["E-0023<br/>Session Service<br/>&amp; FlowTime Studio<br/>★ new"]
    ABSORB["Expert Authoring Surface<br/>absorbed into E-0023"]

    SESS --> CLI --> E21 --> E15 --> PARITY --> E22 --> E23
    ABSORB -.->|draft folder<br/>merged at epic start| E23

    E12["E-0012 Dependency Constraints"]
    E13["E-0013 Path Analysis"]
...
**Studio is the fourth major landing after E-0021.** It sits where the
...
### Why Studio lands after E-0022 and not before

- **E-0022's Pipeline SDK is a prerequisite-shaped sibling.** Pipeline SDK is
...
- **Model fit wants to run in a session.** If fit ships before Studio, it
  ...
  wrapper).
- **E-0015's telemetry work gives Studio something to bind to.** A live
  session without a telemetry binding is a toy. E-0015's `ITelemetrySource`
  ...
  interface extension is small and belongs in E-0015, not Studio.
...
Interface. The first two are substantially absorbed by E-0021 already. The
...
### Why Studio does not displace E-0015 or E-0022

E-0015 is the critical path for the client-telemetry vision. Fit and chunked
...
### E-0021 Svelte Workbench & Analysis Surfaces — **finish as scoped**
...
### E-0022 Model Fit + Chunked Evaluation + Pipeline SDK — **narrow the SDK scope**
...
- **m-E22-01 Model Fit** — land as `POST /v1/fit` (one-shot, same shape as
  ...
  Studio re-homes fit as a session-scoped op in E-0023 (convenience wrapper
...
- **m-E22-03 FlowTime.Pipeline SDK** — **pin to narrow scope.** Wraps the
  ...
  `optimize`, `fit`, `validate`). No session client in E-0022. Session
...
lands in Studio without blocking E-0022's critical path.

### Expert Authoring Surface draft — **absorbed into E-0023**
...
- E-0023 delivers the Session Service, IR, patch vocabulary, WS protocol,
  Rust cache extensions.
- E-0023 delivers the expert editor shell as one milestone (planned:
  m-E23-05).
- `docs/explorations/expert-authoring-surface/` moves to
  `work/epics/E-0023-session-service-and-studio/reference/` at epic start
  (or its reference documents become inline references).

### E-0015 Telemetry Ingestion — **small scope addition, worth naming now**

**Current state:** next after E-0021. Targets Gold Builder → Graph Builder →
first dataset path.
...
**Out of scope for E-0015:** actual streaming adapters (Prometheus, OTEL,
...
**Current state:** sequenced after E-0015. Parity harness between baseline
...
### E-0017 Interactive What-If Mode — **re-homed onto session substrate**
...
This is adapter work, not rewrite. E-0017's scope stays delivered; its
implementation migrates to sit on top of the Studio substrate as one of
Studio's adapter milestones.

### E-0012 Dependency Constraints, E-0013 Path Analysis — **unchanged**
...
- **UI Workbench & Topology Refinement** — substantially absorbed by E-0021
  already. Close the draft or explicitly mark its remaining scope (if any)
  as "post-E-0021 polish." Not a separate epic.
- **UI Analytical Views** — E-0014 was already absorbed here; E-0021's analysis
...
**Rationale:** the three drafts were written before E-0021 or Studio existed.
E-0021 delivered most of what Workbench and Analytical Views proposed.
Question-Driven is a legitimate future epic but depends on Studio.

### E-0014 Visualizations — **confirmed absorbed**

Already noted in ROADMAP.md as absorbed into UI Analytical Views, which is
itself largely delivered by E-0021. No further action.
...
## Proposed E-0023 Shape

Working name: **E-0023 Session Service & FlowTime Studio**. Folder:
`work/epics/E-0023-session-service-and-studio/`.
...
**Depends on:** E-0022 (narrow Pipeline SDK), E-0015 (sliding-window source
...
- **m-E23-03 Re-home analysis ops onto sessions.**
  ...
  wrappers. Large speedup for E-0021 analysis tabs on heavy sweeps.
- **m-E23-04 Workbench + analysis tabs on session.** E-0021 surfaces switch
  from "load a run bundle" to "open a session bound to a run or a model."
  Adapter work if E-0021 preserved stable node IDs in view state.
...
- **m-E23-07 Pipeline SDK session client.** Extends E-0022's narrow SDK with
...
- **D-2026-04-??-???: FlowTime Studio as E-0023, sequenced after E-0022.**
  Records the three points that make this an epic and not just an E-0022
  ...
  editor shell is a new UX surface. Splitting from E-0022 keeps Pipeline SDK
...
- **D-2026-04-??-???: Model Fit lands as one-shot in E-0022, re-homes to
  session op in E-0023.** Calls out the intentional rework so it is not
  surprise scope in E-0023.
- **D-2026-04-??-???: E-0015 adds sliding-window-with-snapshot-id source
  contract.** Not a streaming adapter; an interface extension with one test
  source. Belongs to E-0015, not Studio.
...
- **`ROADMAP.md`**
  - add E-0023 section between E-0022 and the "UI Paradigm Epics" draft
    section
...
- **`work/epics/epic-roadmap.md`**
  - add E-0023 with folder, goal, key milestones, and dependency arrows
  - note Pipeline SDK scope is pinned narrow in E-0022
  - note fit re-home from E-0022 to E-0023
- **`CLAUDE.md` Current Work**
  - add E-0023 entry (status: planning) once the draft spec exists
...
- **`work/epics/E-0022-model-fit-chunked-evaluation/spec.md`**
  - pin m-E22-03 Pipeline SDK to narrow scope
  - flag m-E22-01 fit as "session-rehomes in E-0023"
- **`work/epics/E-0015-telemetry-ingestion/spec.md`**
  - add or extend a milestone with the sliding-window source contract
- **`work/epics/E-0021-svelte-workbench-and-analysis/m-E21-08-*.md`** (when
...
- **`docs/explorations/expert-authoring-surface/`**
  - move to `work/epics/E-0023-session-service-and-studio/reference/` at
...
- m-E21-06 Heatmap View continues to completion on its current spec — **no
  change**
- E-0021's remaining milestones (m-E21-07, m-E21-08) continue to be planned
...
- E-0023 folder created with spec drafted (architecture doc becomes epic
  reference; this plan becomes epic sequencing reference)
- E-0022 spec pinned to narrow Pipeline SDK
- E-0015 spec extended with sliding-window contract milestone

### Deferred until E-0022 wraps

- E-0023 work starts
- expert-authoring-surface folder migration
- E-0017 re-home onto session substrate (post-Studio adapter work)
...
- **Risk: Pipeline SDK gets carved up when Studio re-homes fit.**
  Mitigation: E-0022's narrow SDK scope means fit is a thin wrapper; Studio
  adds a session client alongside rather than reshaping the narrow wrapper.
- **Risk: E-0021 view state ships with positional keys and we pay for it at
...
Starting work on m-E23-01 waits until E-0022 wraps. In the meantime, the
sliding-window source contract addition to E-0015 is the earliest piece of
