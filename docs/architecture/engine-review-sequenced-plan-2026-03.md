...
**Purpose**: Provide an optimally sequenced plan that addresses correctness bugs, analytical gaps, and roadmap blind spots identified in the engine deep review. This document is designed to be reconciled with `docs/ROADMAP.md` and `docs/architecture/epic-roadmap.md` in a follow-up session.
...
| Document | Action |
|----------|--------|
| `docs/ROADMAP.md` | Update to reference this plan. Insert Waves 0-2 before "Near-Term Focus." |
| `docs/architecture/epic-roadmap.md` | Add "Analytical Primitives" as a new near-term epic between Engine Semantics and Dependency Constraints. |
| `docs/architecture/engine-review-findings.md` (March 7) | All items still open. Cross-reference with this plan's Wave 0 and Wave 1. |
| `docs/architecture/engine-deep-review-2026-03.md` (March 23) | Merge from worktree to main. Cross-reference with this plan. |
| `docs/architecture/path-analysis/README.md` | No changes needed. Wave 4 implements it with enrichments from Wave 2. |
| `docs/architecture/anomaly-detection/README.md` | No changes needed. Wave 5 implements it with enrichments from Wave 2. |
| `docs/architecture/overlays/overlays.md` | Minor update: add `wipLimit` and `variability` to overlay descriptor targets after Wave 2 ships. |
| `docs/architecture/dependency-constraints/README.md` | Add note about engine-side enforcement (not just MCP-side). |
