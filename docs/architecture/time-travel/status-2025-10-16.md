...
- ✅ **Available today**: Stable topology semantics, per-bin metrics, derived utilization/latency data, telemetry warning propagation, canonical demo bundles, and provenance metadata.
- ⚠️ **Blocking gaps**:
  1. No API to create telemetry runs from template + time range (UI must script CLI).
  2. No discovery/listing endpoint for available runs or templates.
  3. Absent topology layout hints make consistent graph rendering difficult.
  4. Docs still describe M2.x behaviour, risking confusion during UI planning.

## Immediate Priorities (Proposed)
1. **Ship run orchestration APIs**: Implement `POST /v1/runs` plus supporting GET endpoints that call capture/bundle logic server-side.
