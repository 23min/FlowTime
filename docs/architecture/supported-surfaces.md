...
| Engine API route | `POST /v1/run`, `POST /v1/graph` in [src/FlowTime.API/Program.cs](../../src/FlowTime.API/Program.cs) | Ad hoc direct-YAML evaluation and graph endpoints not used by current first-party UIs | delete | Remove the direct-eval/debug surface and force current workflows through canonical run/query contracts | m-E19-02 | No live `/v1/run` or `/v1/graph` routes remain in runtime code or current docs |
