...
- Time-travel V1 (TT‑M‑03.32.1) is feature-complete: DLQs first-class, template retrofits done, `/state` + `/state_window` live (see `work/epics/completed/time-travel/status-2025-11-23.md`).
...
Before new feature work, address the findings from the engine & schema review (`docs/architecture/reviews/engine-review-findings.md`):
...
**Telemetry Loop & Parity** – Ensure synthetic runs and telemetry replays match within defined tolerances; see `work/epics/telemetry-loop-parity/spec.md`.  

**Scenario Overlays & What-If Runs** – Derived overlay runs for capacity, parallelism, and arrivals experiments with deterministic provenance; see `work/epics/overlays/overlays.md`.  

**Flow-Aware Anomaly & Pathology Detection** – Build on the time-binned DAG model to detect anomalies and recurring flow pathologies (e.g., retry storms, slow drains), group them into incidents, and surface incident-focused stories and dashboards for SREs and stakeholders; see `work/epics/anomaly-detection/spec.md`.  

**Ptolemy-Inspired Semantics & Directors** – Keep time/coordination semantics explicit (e.g., a DiscreteTime director seam) and selectively borrow ideas like modal models, typed ports, and determinacy contracts to future-proof FlowTime’s engine while staying DT-first; see `work/epics/ptolemy/spec.md`.  

## References
- `docs/architecture/reviews/engine-review-findings.md` — Engine & schema review findings (2026-03-07).
- `work/epics/completed/time-travel/status-2025-11-23.md` — Latest epic + global gap status.
...
- `work/epics/epic-roadmap.md` — Architecture epics, their ordering, and links to detailed docs.  
