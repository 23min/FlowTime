...
**Epic:** `work/epics/E-24-schema-alignment/spec.md` | **Status:** in-progress — m-E24-01 Inventory & Design Decisions **complete (2026-04-25, commit `c43e8c0`)**; m-E24-02 Unify Model Type **complete (2026-04-25, commits `131dd35` / `c4064bf` / `3806097` / `dece22f`)**; m-E24-03 Schema Unification **complete (2026-04-25, commit `d6a3263`)**; **m-E24-04 Parser/Validator Scalar-Style Fix in-progress (started 2026-04-25)** on branch `milestone/m-E24-04-parser-validator-scalar-style-fix`. Epic integration branch `epic/E-24-schema-alignment` (created from `main` 2026-04-24). Internal planning ratified **Option E (unify)** on 2026-04-24 per `D-2026-04-24-037`.
...
- m-E24-04 Parser/Validator Scalar-Style Fix — **in-progress (started 2026-04-25)** on branch `milestone/m-E24-04-parser-validator-scalar-style-fix`; mirrored `ParseScalar` fix in both `ModelSchemaValidator` and `TemplateSchemaValidator`; test helper aligned. Pre-state canary: **231 errors** (all `ParseScalar` shape — 92/89/40/10 split); target post-state **0**.
- m-E24-05 Canary Green + Hard Assertion (`TemplateWarningSurveyTests` asserts `val-err=0`; E-23 resume gate)

**ADRs (ratified in m-E24-01):** ADR-E-24-01 Unify the post-substitution model type · ADR-E-24-02 Forward-only regeneration · ADR-E-24-03 Schema declares only consumed fields · ADR-E-24-04 (m-E24-04) `ScalarStyle.Plain` gates `ParseScalar` coercion.
