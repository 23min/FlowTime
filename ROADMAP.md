...
- m-E23-01 Rule-Coverage Audit — **status: in-progress (started 2026-04-26).** Doc-only audit of every rule embedded in `ModelValidator.cs`, `ModelParser.cs`, `SimModelBuilder.cs`, and post-parse orchestration. Per-rule disposition (schema-covered / schema-add / adjunct / parser-justified / drop); schema additions and `ModelSchemaValidator` adjuncts land where the audit needs them; negative-case canary `RuleCoverageRegressionTests` locks coverage in. Replaces the original `m-E23-01-schema-alignment` milestone — that earlier work was already covered by E-24 m-E24-03 / m-E24-05; the unowned piece (the `ModelValidator` rule audit) becomes this milestone's focus. Slug change also avoids collision with E-24's "schema-alignment" slug.
